...
package classes.Items.Armors
{
	...
	public final class LustyMaidensArmor extends Armor {
		...
		override public function canUse(host:Creature):String {
			var baseCanUse:String = super.canUse(host);
			if (baseCanUse != null) {
				return baseCanUse;
			}
			if (host.biggestTitSize() < BreastCup.A) { //{No titties}
				return "You slide the bikini top over your chest and buckle it into place, but the material hangs almost comically across your flat chest.  The cold chain dangles away from you, swaying around ridiculously before smacking, cold and hard into your [nipples].  This simply won't do - it doesn't fit you, and you switch back to your old armor.\n\n";
			}
			if (host.biggestTitSize() < BreastCup.D) { //{Too small titties}
				return "You slide the bikini top over your chest, shivering when the cold chains catch on your nipples, stiffening them nicely. The material nicely accentuates your chest, but there's a definite problem.  Your [chest] aren't big enough!  Sure, they look nice done up in glittering silver and gold trim.  If only the metal wasn't hanging loosely around your underbust, flopping around whenever you move.  It doesn't even look that sexy on you!  You'll need a bigger chest to truly make use of this armor.  For now, you switch back to your old equipment.\n\n";
			}
			var text:String = useTextIntro;
			if ((host.hasCock() && !host.hasSheath()) || host.balls > 0) {
				text += "that it won't fit you or your [if(hascock)[cocks]|[balls]] at all!  <b>You put your old gear back on with a sigh</b>.";
				return text;
			}
			else if (!host.hasVagina()) {
				text += "that it will dig uncomfortably into your featureless groin.  <b>You put your old gear back on with a sigh</b>.";
				return text;
			}
			return baseCanUse;
		}
		...
		override public function useText(host:Creature):String {
			var text:String = useTextIntro;
			text += "your [vagina] is prominently displaying your camel-toe for all to see."
			...
			if (host.vaginas[0].virgin) {
				text+= "a tight seal over your chastity, displaying your womanly status while guarding your maidenhead at the same time.  A smug, smile tugs at the corners of your mouth - who would take your virginity when they can tit-fuck your tits or fuck your butt?";
				if (host.cor < 33) {text += "  Wait, that isn't right...";}
			} else {
				text += "a tight seal over your previously-claimed cunt.  Regret fills you when you realize you could have kept your chastity intact simply by servicing the lusty studs and monsters with your ass and tits."
			}
			if (host.wetness() >= 3) text += "  The moisture you normally drip seems to soak right into the gusset instead of running down your [legs] like normal, giving you a much more chaste appearance in spite of the lewd garments that even now seem to shape your femininity and [butt] into perfectly arousing shapes.";
			...
			host.dynStats("lus", 25, "scale", false);
			return text;
		}
