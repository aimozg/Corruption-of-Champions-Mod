...
package classes.Items.WeaponsRange {
	...
	public class InquisitorsTome extends WeaponRange {
		...
		public function InquisitorsTome() {
			super(new WeaponBuilder("I. Tome", WeaponRange.TYPE_TOME, "Inquisitor's Tome", WeaponRange.TYPE_TOME)
					.withShortName("I. Tome").withLongName("an Inquisitor's Tome")
					.withVerb("nothing")
					.withAttack(0).withValue(2000)
					.withDescription("This red tome is filled with forbidden knowledge, concealed within a detailed treatise on the demon war. The magic within this tome allows you to cast spells using your health after you become too low on mana to cast spells normally.")
					.withPerk(PerkLib.LastResort));
		}
		...
		override public function useText(host:Creature):String {
			...
			if (host.hasPerk(PerkLib.JobSorcerer)) {
				text += "Beyond the words... you feel power in the pages, the ink and the binding. ";
			}
			text += "You reclasp the tome, and hang it ";
			if (host.armorName == "nothing") {
				text += "from your shoulder";
			} else {
				text += "from your belt";
			}
