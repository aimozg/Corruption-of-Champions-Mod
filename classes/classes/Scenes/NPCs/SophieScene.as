...
public function meetSophieRepeat():void {
	...
	clearOutput();
	outputText("SOMETHING SCREWY HAPPENED IN SOPHIE'S MEETING");
	return;
}
...
private function cramANippleInIt():void {
	...
	if (player.biggestLactation() < 2) {
		outputText("She suckles harder and harder until your feel your milk let down.  A pleasant trickle of your breast-milk leaks from your " + player.nippleDescript(0) + " into her mouth.  Sophie's arms close around your back, allowing her to lock herself onto you as she giddily sucks down your liquid nutrients.  The expression on her ");
	...
	//(Medium PG)
		...
		outputText(player.nippleDescript(0) + ".  The suckling goes on nearly as long as it did with the first, until she finishes.");
		if (player.totalBreasts() > 2) {
			if (player.totalNipples() > 2) outputText("She drinks from each of the nipples that adorn your " + player.allBreastsDescript() + " until her belly is full and gurgling.");
...
private function consensualHotSophieDickings():void {
	...
	outputText("With her spread thighs beckoning you so invitingly, there's no way you could resist.  You tear off your " + player.armorName + " and jump into her nest with her, hardening delightfully from your close proximity to the well-endowed woman.  Sophie places a hand ");
	if (player.biggestTitSize() < 1) outputText("on your chest");
	else outputText("between your " + player.allBreastsDescript());
	...
	outputText("Sophie releases you from her bountiful bosom, but you take your time coming back out; you even stop to lick one of her pert nipples.  The harpy takes you by the hips and pulls you on top of her, squeezing your " + player.assDescript() + " for good measure.   With ");
	...
	outputText("your " + player.multiCockDescriptLight() + " begins to poke and prod against her crotch and slightly parted slit.  ");
	...
	if (player.cockTotal() > 1 && player.biggestCockArea() > 232) {
		outputText("  Her hands push away the bloated flesh of your " + player.cockDescript(player.biggestCockIndex()) + " in an effort to better get at your " + player.cockDescript(x) + ".  She grunts, \"<i>Tempting, but too much of a good thing.</i>\"");
	}
	outputText("\n\n");
	...
	outputText("Sophie coos, \"<i>Go on " + player.mf("boy","girl") + ", give your " + player.cockDescript(x) + " a taste.</i>\"  You oblige her request by rocking your hips forward, ");
	if (player.cockArea(x) <= 6) outputText("plunging your " + player.cockDescript(x) + " inside the oversized vagina.");
	else if (player.cockArea(x) <= 150) outputText("burying your " + player.cockDescript(x) + " deep into her massive gash.");
	else outputText("slowly working your " + player.cockDescript(x) + " into her massive gash, finding it barely big enough for you.");
	...
	else outputText(player.cockDescript(x) + " and prostate");
	...
	outputText(" your " + player.multiCockDescriptLight() + " remains.  Sophie giggles, \"<i>");
	if (flags[kFLAGS.FUCKED_SOPHIE_COUNTER] == 0) {
...
private function consensualSophieSexNoFit():void {
	...
	if (player.balls > 0) outputText("your " + player.ballsDescriptLight() + ", careful not to snag them with her talons");
...
private function maleVictorySophieRapeHUGE():void {
	...
	outputText(".");
	if (player.cumQ() >= 500) {
		outputText("  Clenched tightly, you revel in the feeling of thick jizz-globules distorting your " + player.cockDescript(x) + " as they blast out.  It splatters wetly, making a total mess of your curvy victim");
	...
	outputText("  Sophie ");
	if (player.cocks[x].cockLength < 50) outputText("covered the underside of your " + player.cockDescript(x) + " in kisses during your orgasm, but the tingling oral drugs only made it feel better.");
...
private function sophieVictoryPussyGrind():void {
	...
	if (player.biggestTitSize() >= 1) outputText("You pinch a " + player.nippleDescript(0));
	...
	outputText("Girl-cum starts to run down the crack of your " + player.assDescript() + " and a moment later you're cumming.");
	if (player.cockTotal() > 0) {
		outputText("  " + player.SMultiCockDesc() + " explodes, splattering her with seed.  The bird-woman looks at the mess like so much wasted potential, but it just makes you grin harder.");
		if (player.cumQ() < 400) {}
	...
	if (player.vaginas[0].vaginalWetness >= VAGINA_WETNESS_SLAVERING) outputText("Juices squirt from the joined pussies");
	else if (player.vaginas[0].vaginalWetness >= VAGINA_WETNESS_SLICK) outputText("Juices soak the nesting material");
	else if (player.vaginas[0].vaginalWetness >= VAGINA_WETNESS_WET) outputText("Juice trickles from the paired pussies");
...
private function fuckDatClit():void {
	...
	if (player.totalCocks() > 0) {
		outputText("Whiteness erupts from " + player.sMultiCockDesc() + " as the orgasm hits your groin.  ");
	...
	if (player.vaginas[0].vaginalWetness >= VAGINA_WETNESS_DROOLING) outputText("Juice dribbles down your " + player.legs() + " as your body enjoys the treatment.  ");
...
private function tinyDickSupremeSophieLoss():void {
	...
	if (getGame().amilyScene.hasMet() && player.cor > 50) outputText("You imagine Amily's voice nattering away about how terrible the corruption is, but it doesn't help!  Y");
