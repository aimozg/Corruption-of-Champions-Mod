...
private function loseToIzma():void {
	...
	if (player.gender <= 1) {
		...
		if (player.analCapacity() < 26) {
			outputText("You can't help but yell in pain at the sudden sensation of something so huge forcing its way into your " + player.assholeDescript() + ".");
		...
		outputText("Harder and faster she thrusts, building up a rhythm that grows in pace, her balls slapping audibly against your " + player.buttDescript() + " as she bucks back and forth.  You can feel her huge male organ in your depths, rubbing against your prostrate, stretching your inner walls, her boiling erection against your burning heat.  You moan in pleasure; you can't help but enjoy this");
		...
		//[(male)
		if (player.hasCock()) {
			outputText(player.sMultiCockDesc() + " disgorging ");
	...
	//Loss Scene- Herm 
		...
		outputText("\"<i>Dirty little minx, ain't ya?  You wanted this, didn't you...</i>\" Izma teases, shoving two fingers into your moist nether-lips, to test the waters.  The penetration of your needy cunt does serve to make you whimper softly, almost begging to just be filled.  ");
		//(Tight/Virgin vagina)
		if (player.vaginalCapacity() < 26) {
		...
		//(20+ inches)
		else outputText("Izma gives a low whistle at the sight of your " + player.cockDescript(x) + ".  \"<i>Now THAT'S a cock.  Looks like you've got a third leg down there!</i>\"");
...
private function finalIzmaSubmission():void {
	...
	if (player.hasCock() || player.hasVagina()) {
		...
		if (player.hasCock()) {
			outputText(player.sMultiCockDesc() + " stiffen");
	...
	outputText("She keeps you held in place as she positions the tip of her cock against your " + player.assholeDescript() + ".  With no warning, she forces her meat-pole into you, making you squeal in delight from the familiar heat of her cock.  ");
	...
	outputText("As if sensing your complete submission, Izma's hands trail down toward your " + player.breastDescript(0) + ", mauling and groping at them, tweaking your " + player.nippleDescript(0) + "s in an almost-painful fashion.  It only manages to turn you on further though, groaning and squirming from intense pleasure.  Izma makes you orgasm first, your muscles twitching and spasming as you briefly lose control over yourself");
	if (player.hasVagina() || player.hasCock()) {
		outputText("; ");
		if (player.hasCock()) {
			outputText(player.sMultiCockDesc() + " begins spurting jizz all over the sandy shore");
			if (player.hasVagina()) outputText(" and ");
		}
...
private function useVagooOnIzmaWin():void {
	...
	//(non-taur and height <= 4')
	else outputText("but small as you are, you can't stop her as she reaches up and grabs onto your " + player.buttDescript() + ", then begins bouncing you like a goblin cocksleeve.");
	outputText("\n\n");
...
private function nonFightIzmaSmexCUNTPUSSYSNATCHQUIM():void {
	...
	outputText("You nod and remove your " + player.armorName + ", not doing it with quite as much showmanship as Izma herself.  You expose your " + player.allBreastsDescript() + " to the air; your " + player.nippleDescript(0) + "s grow stiff from the breeze.  The bottom half of your clothing comes off, exposing your " + player.vaginaDescript(0));
	...
	if (player.hasCock()) outputText(" and " + player.sMultiCockDesc() + " shooting a weak load across her chest");
...
private function izmaLakeDominate():void {
	...
	//[Female]
		...
		if (player.isTaur()) outputText("your weight pinning her to the sand and preventing her from taking control.");
		///(non-taur and height > 4')
		else if (player.tallness > 48) outputText("and pin her hands above her head to stop her from trying to change the position. Her heaving breasts rub against your " + player.chestDesc() + " as you ride her in this posture.  She needs to know who's in charge here, after all.");
		//(non-taur and height <= 4')
		else outputText("but small as you are, you can't stop her as she reaches up and grabs onto your " + player.buttDescript() + ", then begins bouncing you like a goblin cocksleeve.");
		outputText("\n\n");
...
private function submitToLakeIzma():void {
	...
	//(herm)
		...
		//(20+ inches)
		else outputText("Izma gives a low whistle at the sight of your " + player.cockDescript(0) + ".  \"<i>Now THAT'S a cock.  Looks like you've got a third leg down there!</i>");
		outputText("\n\n");
...
private function followerIzmaTakesItInPooper():void {
	...
	if (player.hasCock()) {
		...
		//[(if player has balls)
		if (player.balls > 0) outputText(" and " + player.sackDescript());
		outputText(" until her dark-red cock is lined up with your " + player.multiCockDescriptLight() + ".\n\n");
	
		outputText("She takes your " + player.cockDescript(0) + " and presses it against her own, holding both of them together in one hand and rubbing slightly.  Her other hand reaches down below, to rub her double set of balls");
		//[(if player has balls)
		if (player.balls > 0) outputText(" against your own " + player.ballsDescriptLight());
		//(if no balls and vag)
	...
	//(if player has penis)
		...
		if (player.cocks[0].cockLength < 14) {
			outputText("Izma grabs your " + player.cockDescript(0));
			if (player.cockTotal() > 1) outputText(" first, then your other " + player.cockDescript(1) + ", synchronizing her movements");
			outputText(", and starts jerking you off.  It feels incredible, and when she says, \"<i>You've been a good beta for me, but now I want to see you come,</i>\" you can't help but obey; you orgasm messily onto your own " + player.chestDesc());
			//(huge cum)
		...
		//(else if penis 0 length <16 inches)
			...
			outputText("When Izma guides " + player.sMultiCockDesc() + " between her breasts, you're not sure what you're feeling at first; smooth in one direction, rough in the other direction.  Pushing her breasts tight around your " + player.multiCockDescriptLight() + ", she starts to move up and down.  The contrast quickly brings you over the edge and you cum all over her face and breasts, ");
			//[(normal cum)
...
private function followerIzmaTakesIt69():void {
	...
	//[(Male/herm)
	if (player.hasCock()) {
		outputText("gorging herself on your " + player.cockDescript(0) + ", her tongue wrapping around it and caressing it like jewelry");
		//[(if herm)
		if (player.hasVagina()) outputText(" while pushing two fingers into your " + player.vaginaDescript(0));
	}
	//(Female)
	else if (player.hasVagina()) {
		outputText("licking and eating you out, paying special attention to your " + player.clitDescript());
	}
	outputText(".\n\n");
	...
	else outputText("to push your " + player.clitDescript());
...
private function inCampRideIzmasDickDongTheWitchIsDead():void {
	...
	outputText("Slowly and sensuously you remove your " + player.armorName + ", exulting in the way Izma hungrily drinks in every inch of exposed " + player.skinFurScales() + ".  Her eyes devour your " + player.allBreastsDescript() + ", then slide eagerly down to your crotch to feast upon");
	if (player.hasCock()) {
	...
	if (player.hasCock()) {
		outputText("; " + player.sMultiCockDesc() + " spraying all over her belly, breasts and face");
	}
