...
public function useTentacleJojo():void {
	...
	outputText("You drop it, letting it smack into the floor with a wet-sound 'SPLAT' that creates a puddle of pre-cum on the ground.  With your hands free, it is easy to quickly remove your " + player.armorName + " and toss it back towards your " + camp.homeDesc() + ".  By the time you turn back around, his snake-like member is curling up your thigh, towards your " + player.assholeOrPussy() + ".  ");
	...
	if (player.totalCocks() > 0) {
		...
		if (player.cockTotal() == 1) {
			...
			if (player.balls > 0) {
				...
				outputText(" your " + player.cockDescript(0) + " in a tight vise of pleasure.  The other two tentacles slide under, each taking a " + player.ballDescript() + " partially inside their 'mouths' and massaging them softly.");
				outputText("\n\n");
			}
			//(1 dick no balls = 
			else {
				...
				outputText(" your " + player.cockDescript(0) + " in a tight vise of pleasure.  The other two tentacles slide under, ");
				if (!player.hasVagina()) outputText("rubbing back and forth along your taint and smearing you with their slime.");
				else {
					outputText("rubbing against your thighs and smearing them with lubricant while one of them easily sucks your " + player.clitDescript() + " inside.");
					//(If big clit – 
					if (player.getClitLength() >= 4) outputText("  You nearly cum on the spot from the clitoral stimulation; it's like you have a super-sensitive cock being licked by a thousand tongues.");
				}
				outputText("\n\n");
			}
		}
		...
		if (player.cockTotal() == 2) {
			...
			//(2 dicks no balls) 
				...
				outputText(" your " + player.multiCockDescriptLight() + " in tight vises of pleasure.  The remaining tentacle slides underneath, seeking ");
				if (player.hasVagina()) {
					outputText("out your " + player.clitDescript() + " and sucking it inside.");
					//(If big clit – 
					if (player.getClitLength() >= 4) outputText("  You nearly cum on the spot from the clitoral stimulation; it's like you have a super-sensitive cock being licked by a thousand tongues.");
				}
		...
		if (player.cockTotal() >= 3) {
			...
			outputText(" your " + player.multiCockDescriptLight() + " in tight vises of pleasure.  The sensation is overwhelmingly pleasurable, like having three cocks being licked and sucked by a thousand tongues.");
			//(if extra: 
	...
	if (player.totalCocks() == 0 && player.hasVagina()) {
		outputText("One of Jojo's tail-tentacles slides underneath his body, shooting forwards to seek out your " + player.clitDescript() + ".  It pauses an inch or two away and opens up a horrific orifice full of wriggling tentacles and slick fluids.  The next moment it lunges forwards and sucks it inside.");
		//(If big clit – 
		if (player.getClitLength() >= 4) outputText("  You nearly cum on the spot from the clitoral stimulation; it's like you have a super-sensitive cock being licked by a thousand tongues.");
	...
	if (player.totalCocks() > 0) {
		...
		if (player.cockTotal() == 1) outputText("  The tentacle on your " + player.multiCockDescriptLight() + " seems appreciative, and begins sucking to swallow down all your pre.");
		if (player.cockTotal() > 1) outputText("  The tentacles on your " + player.multiCockDescriptLight() + " seem appreciative, and begin sucking to swallow down all your pre.");
	}
	if (player.hasVagina()) outputText("  Your " + player.vaginaDescript(0) + " spasms around the invader, clenching uncontrollably as it's plowed by the thick vine-prick.");
	outputText("\n\n");
	...
	if (player.biggestLactation() >= 2) {
		...
		outputText(".  Jojo's dick-mouths latch on and suck hard, pulling your " + player.nippleDescript(0) + "s deep into their internal reservoir and stimulating your breasts to release a torrent of milk.  ");
		...
		if (player.breastRows.length > 1) {
			//(NewPG if multirow)
			outputText("\n\nThere is nothing to do but enjoy the sensations until your " + player.breastDescript(0) + " are emptied, but before that can happen, they detach, soaking your chest with excess milk, and move down to the next row.  It becomes clear they will keep milking you, moving from row to row to give each a chance to recover, so that they can feed indefinitely...");
		}
		outputText("\n\n");
	}
	//(NEITHER - JUST NIPS) 
	...
	if (player.hasVagina()) {
		...
		if (player.cockTotal() <= 2 && player.balls == 0) {
			outputText("The tentacle locked around your " + player.clitDescript() + " bulges out a bit as your clit throbs from the orgasm, before it sucks harder, making your clitty even larger.  You squeal from the raw sensation until the tentacle pops off, satisfied, leaving your clit looking like an angry red cock.  ");
	...
	if (player.cockTotal() > 0) {
		if (player.cockTotal() == 1) {
			outputText("The tentacle working over your " + player.multiCockDescript() + " clamps down and wriggles deliciously around you.  You can't help but cum from the assault, feeding the tentacle your hot spoogy load.  It drinks it down eagerly");
			if (player.cumQ() >= 100) {
		...
		if (player.cockTotal() > 1) {
			outputText("The tentacles working over your " + player.multiCockDescript() + " clamp down and wriggle deliciously around you.  You can't help but cum from the assault, feeding the tentacles your hot spoogy load.  They drink it down eagerly");
			if (player.cumQ() >= 100) {
	...
	if (player.biggestLactation() >= 2 && nippleSucking) {
		outputText("Your " + player.breastDescript(0) + " squirt milk even harder, pushing the tentacles clear off you with the force of your lactation.  Your " + player.nippleDescript(0) + "s look like ");
...
private function milkJojoFirst():void {
	...
	if (flags[kFLAGS.PC_COCK_MILKED_COUNTER] > 0) outputText(", even though it reeks of the spunk you've spilled into the milker's mechanical embrace");
	...
	//(Vagoozle!)
		...
		if (player.getClitLength() > 3) outputText("Your other hand grabs your penis-sized 'button' and begins to jack it, the action sending tremors of sensation through your " + player.hipDescript() + ".  ");
	...
	if (player.hasCock()) outputText(", squeezing your " + player.cockDescript(x) + " with agonizingly pleasurable pressure");
...
//this.semiglobalReferencer.jojoRape = jojoRape;
		...
		private function jojosSecondRape():void {
			...
			dynStats("lib", -10, "cor", 4);
			if (player.gender == 1) {
				outputText("You force Jojo over a log, running your hands through his fur and slapping his ass.  He grunts, but it's impossible to tell if it's in frustration, anger, or arousal.  You quickly force yourself back into his ass, finding it noticeably stretched from your last incursion.  ");
...
public function corruptJojoEncounter():void {
	...
	if (flags[kFLAGS.JOJO_STATUS] == 2) outputText("his mousey visage twisted into a ferocious snarl.  \"YOU!\" he screams, launching himself towards you, claws extended.");
	if (flags[kFLAGS.JOJO_STATUS] == 3) outputText("unsteady on his feet, but looking for a fight!");
	if (flags[kFLAGS.JOJO_STATUS] == 4) outputText("visibly tenting his robes, but intent on fighting you.");
	if (flags[kFLAGS.JOJO_STATUS] == 5) outputText("panting and nude, his fur rustling in the breeze, a twitching behemoth of a cock pulsing between his legs.");
...
public function getBackYouMenu():void {
	...
	if (player.hasItem(consumables.BIMBOLQ)) {
		outputText("You look at the Bimbo Liqueur in your hand.");
	} else {
		outputText("You hesitate for a moment, contemplating your choices.");
	}
...
public function keepChanging():void {
	clearOutput();
	jojoSprite();
		outputText("Of course you might. It's impossible to tell what the future holds, and you might need him to if the thief becomes too much of a problem. You decide to keep your options open.");
	doNext(playerMenu);
}
...
public function stopChanging():void {
	clearOutput();
	jojoSprite();
		outputText("Of course you won't. You're sure Jojo could handle the thief, but you don't want to put him in that kind of dangerous position in the first place. You decide that you won't change your mind.");
...
private function getAnallyFuckedByMouseYouSlut():void {
	...
	player.buttChange(monster.biggestCockArea(), true);
...
private function getVagFuckedByMouse():void {
	...
	if (flags[kFLAGS.JOJO_VAGINAL_CATCH_COUNTER] == 0) {
		...
		player.cuntChange(monster.biggestCockArea(), true);
