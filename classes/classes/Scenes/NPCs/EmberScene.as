...
//Approaching Ember (Z)
		...
		public function emberCampMenu():void
		{
			...
			addButton(2, "Drink Blood", bloodForTheBloodGod).hint("Ask Ember if " + emberMF("he", "she") + "'s willing to give you some of " + emberMF("his", "her") + " blood to gain powers.");
...
//Approach for sex - initial output when selecting [Sex] menu (Z)
		private function emberSexMenu(output:Boolean = true):void
		{
			...
			if (flags[kFLAGS.EMBER_GENDER] >= 2 && player.hasCock() && player.lust >= 33) addButton(6, "Penetrate Her", penetrateEmbrah).hint("Penetrate Ember vaginally!");
...
//Get Milk
		private function getMilkFromEmber():void
		{
			...
			if (emberAffection() <= 25) {
				...
				if ((player.hasStatusEffect(StatusEffects.DragonBreathCooldown) && !player.hasPerk(PerkLib.DraconicLungsEvolved)) || ((player.hasStatusEffect(StatusEffects.DragonDarknessBreathCooldown) || player.hasStatusEffect(StatusEffects.DragonFireBreathCooldown) || player.hasStatusEffect(StatusEffects.DragonIceBreathCooldown) || player.hasStatusEffect(StatusEffects.DragonLightningBreathCooldown)) && !player.hasPerk(PerkLib.DraconicLungs))) {
...
//Get Blown - put your dick in the knife drawer, it'll be fun! (Z, with reservation)
		private function stickDickInKnifeDrawer():void
		{
			...
			if (flags[kFLAGS.EMBER_GENDER] == 1 || flags[kFLAGS.EMBER_GENDER] == 3) outputText("  " + emberMF("His", "Her") + " shaft is already hard as rock, and sometimes you can feel it poke your [feet], smearing small beads of Ember's barely contained pleasure on your " + player.skinFurScales() + ".");
...
//Get Penetrated - also horse-proof, sorry folks! (Z)
		private function getPenetratedByEmberLastSexSceneWoooo():void
		{
			...
			outputText("\n\nEmber leans over you, panting, " + emberMF("his", "her") + " hot breath mere inches from your [face], gazing at you through half-lidded, lust-addled eyes.  \"<i>You feel so good... so hot... so sexy...</i>\" Ember says mid-pant.  You pant right back, reaching up and wrapping your arms around " + emberMF("his", "her") + " neck, staring back at " + emberMF("him", "her") + " with the same expression and telling " + emberMF("him", "her") + " you think " + emberMF("he", "she") + " feels just as good.");
			...
			if (player.hasPerk(PerkLib.HistoryWhore) || player.hasPerk(PerkLib.HistorySlut)) outputText("  Using the skills you've honed, you make the muscles in your vagina ripple and wrinkle, teasing the cock caught inside you in a way few women can.");
		...
		public function sleepWithEmber():void {
			...
			//High affection
				...
				if (chooser == 0 && flags[kFLAGS.EMBER_MILK] > 0 && flags[kFLAGS.TIMES_SLEPT_WITH_EMBER] > 0) { //Milk time!
					...
					outputText("\n\nBreathing calmly, you turn around, gently sloshing through the water until you are facing your draconic lover.  A quick glance towards Ember confirms your suspicion; " + emberMF("he", "she") + " is almost giddy.  However Ember tries to at least look professional in " + emberMF("his", "her") + " task; " + emberMF("he", "she") + " starts by rubbing your " + breastDescript(0) + ", lingering a little longer than necessary on your " + nippleDescript(0) + ".");
					...
					outputText("\n\nEmber smiles.  \"<i>Good.</i>\" Slowly but surely making " + emberMF("his", "her") + " way towards your " + (player.hasCock() ? multiCockDescriptLight() : "") + (player.hasCock() && player.hasVagina() ? " and " : "") + (player.hasVagina() ? vaginaDescript() : "") + (player.gender == 0 ? "groin" : "") + ". ");
					...
					outputText("\n\n\"<i>O-okay... Fine...</i>\" Ember concedes, finally reaching for your " + (player.hasCock() ? multiCockDescriptLight() : "") + (player.hasCock() && player.hasVagina() ? " and " : "") + (player.hasVagina() ? vaginaDescript() : "") + (player.gender == 0 ? "genderless crotch" : "") + ".  You knead and massage " + emberMF("his", "her") + " " + (flags[kFLAGS.EMBER_ROUNDFACE] == 0 ? "scalp" : "hair") + ", then start bringing your hands down, rubbing " + emberMF("his", "her") + " shoulders and neck before moving on to " + emberMF("his", "her") + " " + (flags[kFLAGS.EMBER_GENDER] >= 2 || flags[kFLAGS.EMBER_MILK] > 0 ? "flat chest" : "sizable breasts") + ", gently playing with the dragon's nipples.");
