...
override public function urtaLove(love:Number = 0):Boolean {
	...
	if (flags[kFLAGS.URTA_PC_LOVE_COUNTER] == 1) {
		...
		if (love > 0 && flags[kFLAGS.AMILY_VISITING_URTA] == 0 && amilyScene.amilyInCampPure()) flags[kFLAGS.AMILY_NEED_TO_FREAK_ABOUT_URTA] = 1;
		return true;
	}
	else return false;
}
...
public function urtaBarDescript():Boolean {
	...
	if (flags[kFLAGS.URTA_PC_LOVE_COUNTER] == 1) {
		if (flags[kFLAGS.URTA_DRINK_FREQUENCY] == -1 || !urtaDrunk()) outputText("Urta is sitting at her usual table, sipping a glass of wine and wearing a form-fitting evening gown of shimmering black.  She looks up at you, happiness filling her eyes when she notices you entering the bar.");
		//(DRUNK)
	...
	if (flags[kFLAGS.URTA_PC_LOVE_COUNTER] == 1)
	{
		...
		if (flags[kFLAGS.URTA_DRINK_FREQUENCY] == -1 || !urtaDrunk()) {
			...
			if (player.hasCock() && !player.isTaur()) {
				...
				addDisabledButton(3, "Vixen & Cream", "This scene requires you to have cock. It can't accommodate taurs.");
			}
...
private function drinkUrtasBoozeLetHer():void {
	...
	else if (player.biggestTitSize() > 1) outputText(player.biggestBreastSizeDescript());
	else outputText(player.nippleDescript(0) + "s");
	...
	if (player.cockTotal() > 0) outputText(" and " + player.multiCockDescript());
	...
	if (player.totalCocks() > 0) {
		...
		outputText("our " + player.multiCockDescriptLight() + " squirts hard, unloading a batch of sticky jism under your ");
		...
		else outputText("Your " + player.assholeDescript() + " clamps down tightly, spasming in pleasure around the invader.");
	}
...
private function dualUrtaMasturbation():void {
	...
	else outputText("bare groin and " + player.assholeDescript());
	outputText(".  ");
	if (player.totalCocks() > 0) {
	...
	if (player.totalCocks() == 1) outputText("Your hands caress your " + player.cockDescript(0) + " feverishly, tweaking every ounce of pleasure from the bloated rod.  ");
	if (player.totalCocks() > 1) outputText("Your hands caress your " + player.multiCockDescript() + " in turn, dancing back and forth from rod to rod as you attempt to play with every available pleasure outlet.  ");
	...
	if (player.totalCocks() > 0) {
		if (player.cumQ() < 250) {
			...
			outputText("your " + player.multiCockDescriptLight() + " empties itself.  ");
		}
			...
			if (player.totalCocks() > 1) outputText("each of ");
			outputText("your " + player.multiCockDescriptLight() + ", blasting into the air before splattering onto the ground between you and Urta.  It forms a massive puddle, spreading as you pump gob after gob of cum into it until the sticky spooge is soaking your " + player.legs() + " and Urta too.");
		}
		else {
			outputText("Jism fountains from ");
			if (player.totalCocks() > 1) outputText("each of ");
			outputText("your " + player.multiCockDescriptLight() + ", flooding the small alleyway with an unholy amount of cum.  In no time you've painted Urta and yourself from head to toe, and the alleyway sloshes with the stuff as your production overwhelms the ground's ability to absorb liquid.  ");
		}
	}
	if (player.hasVagina()) {
...
public function urtaFuckHer(afterBefriending:Boolean = false):void {
	...
	if (player.totalCocks() == 0) {
		...
		if (player.cockTotal() > 0) {
			if (player.cockTotal() > 2) outputText("a pair of your " + player.multiCockDescriptLight() + " against her pussy and asshole.  ");
			else if (player.cockTotal() == 2) outputText("your " + player.multiCockDescriptLight() + " against her pussy and asshole.  ");
			else if (player.cockTotal() == 1) outputText("your " + player.multiCockDescriptLight() + " against her pussy.  ");
		}
		//DOES IT FITZ
		if (player.cockArea(x) < urtaCapacity()) {
			...
			if (cocks > 1 && y >= 0) outputText(", " + player.cockDescript(x) + ", and " + player.cockDescript(y) + ".");
			else outputText(" and " + player.cockDescript(x) + ".");
			...
			if (cocks > 1 && y >= 0) outputText("  Your " + player.cockDescript(y) + " feels like it's about to explode inside her asshole when her sphincter clamps down tightly, acting like the world's tightest cock-ring.");
			outputText("\n\n");
		...
		//(TOO BIG)
			...
			if (cocks == 1) {
				...
				outputText("Urta pants out, \"<i>Ah ahhh ahhhh,</i>\" and cums, her flare ballooning out until it's twice as wide as her shaft, a wave of white jism escaping from her horse-cock.  You gather some in your hand and slather it over your own " + player.multiCockDescript() + ", and after a few moments of fevered masturbation, you start cumming with her.  Urta happily squirms against you, painting herself with jism and soaking her clothes with the stuff.  ");
			...
			if (cocks > 1) {
				...
				outputText("Urta pants out, \"<i>Ah ahhh ahhhh,</i>\" and cums, her flare ballooning out until it's twice as wide as her shaft, a wave of white jism escaping from her horse-cock.  You gather some in your hand and slather it over your own " + player.multiCockDescriptLight() + ", and after a few moments of fevered masturbation, you start cumming with her.  Urta happily squirms against you, painting herself with jism and soaking her clothes with the stuff.  ");
				if (player.cumQ() < 250) outputText("You join in, though your own contribution seems meager by comparison.");
				else if (player.cumQ() < 500) outputText("You join in, and manage to match her spurt for spurt, until she's lying in a thick puddle of the stuff.");
				else outputText("You join in, and make her orgasm look like a trickle compared to the flood of spunk you unleash onto her.  By the time the pair of you finish, she's soaked from head to toe, and lying in a nicely sized puddle of the stuff.");
...
internal function blowUrtaUnderTable():void {
	...
	if (player.biggestTitSize() >= 2) outputText("dragging between your " + player.allBreastsDescript());
...
internal function takeUrtaInTheButtPublically():void {
	...
	if (player.totalCocks() > 0) {
		...
		if (player.totalCocks() > 1) outputText("each of ");
		outputText("your " + player.multiCockDescriptLight() + " from swelling to full erectness.  A bead of pre forms at ");
		...
		outputText(" of your " + player.multiCockDescriptLight() + ", squeezed out from your prostate.  ");
	}
	if (player.hasVagina()) {
		outputText("Heat spreads through your loins as your " + player.vaginaDescript(0) + " engorges with blood, allowing your " + player.clitDescript() + " to ");
	...
	//(NIPPLES:
	outputText("Your " + player.nippleDescript(0) + "s tent up under your " + player.armorName);
	...
	else outputText("you find yourself sliding your hands under your " + player.armorName + " and pinching your " + player.nippleDescript(0) + "s, putting on a show for the crowd.");
	...
	if (player.biggestTitSize() >= 1) {
		outputText("  Your " + player.allBreastsDescript() + " are jiggling and bouncing wildly, ");
	...
	if (player.cockTotal() > 0) {
		if (player.cockTotal() > 1) {
			outputText("  Steady drizzles of cum begin dripping from each of your " + player.multiCockDescriptLight() + " as she pounds away, rubbing hard against your prostate.");
		}
		else {
			outputText("  Steady drizzles of cum begin dripping from your " + player.multiCockDescriptLight() + " as she pounds away, rubbing hard against your prostate.");
		}
	}
...
private function rideUrtaTenderFemale():void {
	...
	if (player.totalCocks() > 0) {
		if (player.totalCocks() == 1) outputText("  Your " + player.multiCockDescriptLight() + " is raging hard and trembling, but ignored for now.");
		else outputText("  Each of your " + player.multiCockDescriptLight() + " is raging hard and trembling, but ignored for now.");
	}
	...
	if (player.biggestTitSize() >= 5) {
		outputText("Your " + player.breastDescript(0) + " mash into the fox's own, your " + player.nippleDescript(0) + "s ");
		...
		if (player.totalCocks() > 0) {
			if (player.cocks[0].cockLength > 15) {
				if (player.totalCocks() == 1) outputText("  Your " + player.multiCockDescriptLight() + " pushes up between the jiggling breasts, slicking them with your pre-cum as it begins to get tit-fucked by both of your trembling forms.");
				else outputText("  Each of your " + player.multiCockDescriptLight() + " pushes up between the jiggling breasts, slicking them with your pre-cum as they begin to get tit-fucked by both of your trembling forms.");
			}
		}
		outputText("\n\n");
	}
	...
	if (player.totalCocks() > 0) {
		if (player.cockTotal() == 1) {
			outputText("Your own " + player.multiCockDescriptLight() + " cums as well, blasting seed over both your ");
			...
			outputText("Each of your own " + player.multiCockDescriptLight() + " cums as well, blasting seed over both your ");
			if (player.cocks[0].cockLength > 15 && player.biggestTitSize() >= 5) outputText("jiggling tits");
			else outputText("bellies");
			outputText(", glazing them in slippery goo.");
		}
	...
	if (player.totalCocks() > 0) {
		outputText(" and ignoring the hot, wet mess your " + player.multiCockDescriptLight() + " ");
...
private function dudeFuckingUrtasCooch():void {
	...
	outputText("You let her know that you want her just the way she is and climb into bed behind her.  She watches you apprehensively over her shoulder as you pull out your " + player.multiCockDescriptLight() + " and start lining yourself up to fuck her doggie-style.  ");
	if (cocks > 1 && y >= 0) outputText("Your secondary " + player.cockDescript(y) + " wobbles directly behind her back-door and Urta's eyes widen a bit when she realizes you're going to double-penetrate her.  ");
	if (cocks > 2) outputText("The remainder of your " + player.multiCockDescriptLight() + " fan out, ready to rub themselves against her butt-cheeks once you push inside.  ");
	...
	outputText("Obligingly, you ease your hips forward, reeling at the heat of her cunt-lips as they welcome your " + player.cockDescript(0) + "'s " + player.cockHead() + " with slippery moisture.  ");
	...
	outputText("Urta cums first.  Her cries of pleasure are muffled as she shoves her face into a pillow, screaming in ecstasy.  The warmth around your " + player.cockDescript(x) + " seems to grow that much hotter as it squeezes you, wringing you tightly from root to tip.  ");
	if (cocks > 1 && y >= 0) outputText("The intensity and pressure in her back-door is even worse, and your " + player.cockDescript(y) + " is nearly in pain from her asshole's tight, pulsating grip.  ");
	...
	if (cocks > 1 && y >= 0) outputText("The " + player.cockDescript(y) + " you've crammed in her ass does not disappoint either, matching its brother squirt for squirt as it spends your seed with reckless abandon.  ");
	if (cocks > 2) outputText("Her butt and tail are covered with the stuff as each of your remaining " + player.multiCockDescriptLight() + " splatters her with spooge, covering her fur in white sludge.  ");
	...
	if (player.biggestTitSize() >= 1) outputText("your own " + player.breastDescript(0));
...
private function tenderTakeItUpTheAssFromUrta():void {
	...
	outputText("Her cheeks color when you give her a nod and begin lowering your " + player.buttDescript() + " towards her pillar of horse-flesh. She gasps quietly when the tip begins to push against your " + player.assholeDescript() + ".  ");
	...
	if (player.totalCocks() > 0) {
		...
		outputText("your " + player.multiCockDescriptLight() + " hardens near instantly from the forced stimulation.");
	}
	...
	if (cocks > 0) {
		...
		if (cocks > 1) outputText("each of ");
		outputText("your " + player.multiCockDescriptLight() + " droops onto her belly, half-hard from the pressure on your prostate, but going a little soft from the lack of attention.");
		if (player.cocks[0].cockLength >= 15) {
			outputText("  Urta winks and guides ");
			if (cocks > 1) outputText("one of ");
			outputText("your " + player.multiCockDescriptLight() + " between her tits");
	...
	outputText("You bounce on top of your fox-lover's equine rod, spearing your " + player.assholeDescript() + " on it over and over.  ");
...
public function eatUrtaOutNomNomPussy():void {
	...
	outputText("Finished at last, Urta's member rapidly deflates, the scent of her cum on your " + player.hairDescript() + " and her juice on your " + player.face() + " lingers.  You slide up into the booth next to your blissed out lover.");
...
public function blowUrtaUnderTheTableLuv():void {
	...
	outputText("\n\nThumping down on your back, you're blissfully aware of the sweet air filling your lungs.  In a moment of reflection, you realize you can taste the salty flavor of Urta's seed on your lips and mouth, and the retracting horse-cock is ");
	if (player.biggestTitSize() >= 1) outputText("dragging between your " + player.allBreastsDescript());
	else outputText("dragging across your chest");
...
private function rideUrtasCoochLoveyDovey():void {
	...
	if (player.cockTotal() > 0) {
		...
		outputText(" to cover your " + player.multiCockDescriptLight() + " with, explaining that she doesn't want to have to mop up afterward, and you grudgingly agree.  ");
	}
	...
	outputText("You bottom-out, but immediately begin rocking your hips, working your lover's twitching rod deep inside your " + player.vaginaDescript(0) + ".  ");
	...
	if (player.cockTotal() > 0) {
		...
		outputText("our " + player.multiCockDescriptLight() + " spurts, unloading waves of cream into the ");
		if (player.cockTotal() == 1) {
...
private function rideUrtaInButtAtHomeLove():void {
	...
	if (player.totalCocks() > 0) {
		...
		outputText("our " + player.multiCockDescriptLight() + " has filled its sheath with a golfball sized globule of pre-cum, and you can feel your body clenching and squeezing, your ");
	...
	//(EVERYBODY ELSE)
		...
		if (player.totalCocks() > 0) {
			...
			if (player.cumQ() >= 750 && player.cumQ() < 1250) outputText("  By the time you're done the balloon is the size of a basketball and looking dangerously close to bursting.");
				...
				if (player.cumQ() >= 1700) outputText("  You fill it fuller and fuller, until it's bigger than you thought possible.  All at once the material gives out, soaking Urta from the tits down in cum, while your " + player.cockDescript(0) + " continues to pump even more onto her.  So much for not making a mess!");
			}
		}
		outputText("\n\n");
	}
...
private function oralFiestyUberExplosionUrta():void {
	...
	if (player.gender == 2 || (player.gender == 3 && rand(2) == 0)) {
		...
		if (player.totalCocks() > 0) {
			...
			outputText("our " + player.multiCockDescriptLight() + " drools lamely, ");
	...
	if (player.balls > 0) {
		outputText("your " + player.ballsDescriptLight() + " and ");
	}
	...
	if (player.balls > 0) outputText(player.ballsDescript() + ".  Slithering over and around them, she bathes your sack in her spit before moving up to your " + player.cockDescript(x) + ".");
	...
	outputText("Muffled, excited moans reach your ears as you work on your lover's pole.  The fox is clearly enjoying herself, and her sounds of pleasure cut off as she rams your " + player.cockDescript(x) + " deep into her throat.  She gurgles noisily while her tongue slides as far down your shaft as possible, licking at your ");
	if (player.balls > 0) outputText(player.ballsDescriptLight());
	else if (player.hasVagina()) outputText(player.vaginaDescript(0));
...
private function urtasCoochiNeedsFuckedLove():void {
	...
	//[Everybody Else]
		...
		outputText("You rock back and forth, sliding your shaft through her spasming muscles, reveling and panting in orgiastic bliss.  Your " + player.cockDescript(0) + " dumps its gooey cargo deep into your lover's canal with barely audible squishing noises.  ");
		if (player.cumQ() < 250) {}
		else if (player.cumQ() < 700) outputText("Her belly puffs up slightly by the time you finish, thanks to your body's copious cum production.  ");
		else if (player.cumQ() < 1000) outputText("Her belly swells larger and larger as you continue to pump your seemingly endless seed into her.  Urta groans, looking a few months pregnant by the time your overproductive " + Appearance.cockNoun(CockTypesEnum.HUMAN) + " stops spewing inside her.  ");
		else {
			outputText("Her belly swells larger and larger, bloating as you blast thick torrents of seed inside her, filling her completely while leftover jism squirts out around your " + player.cockDescript(0) + ".  ");
			if (player.cumQ() < 3000) outputText("By the time you finish, she looks VERY pregnant and is moaning with a mixture of pleasure and discomfort.  ");
			...
			if (player.cumQ() > 5000) outputText("Not content with such an easy release, your " + player.cockDescript(0) + " unloads the last of its cream until the bed and floor are both covered under inches of your gooey cum.  ");
		}
...
private function watchTwoHotBitchesAndJerkIt():void {
	...
	outputText("You open your " + player.armorName + " and sit down on a crate no more than a few feet away from the orally fixated couple, fishing out " + player.sMultiCockDesc());
	if (player.hasVagina()) outputText(" and exposing your " + player.vaginaDescript(0));
	...
	outputText("You stroke " + player.sMultiCockDesc());
	if (player.hasVagina()) outputText(" and touch your " + player.vaginaDescript(0));
	...
	outputText("You pump hard on " + player.sMultiCockDesc());
	...
	if (flags[kFLAGS.TIMES_CAUGHT_URTA_WITH_SCYLLA] == 1) outputText("\"<i>We need to do thish again!</i>\"");
...
private function makeUrtaSitOnYourLapWithScylla():void {
	...
	outputText("Scylla's neck is pulled tight as Urta's urethra is stretched wide by huge bulges of cum.  You can actually feel them sliding past the fingers you've dug into her sheath.  No sooner than one squeezes past is another on its way.  You wonder how Scylla can handle such incredible volumes and the only answer you can think of is magic.  Whatever the case, her swallowing gets noisier and Urta starts bouncing even faster atop your " + player.cockDescript(x));
	if (y != -1) outputText(" and " + player.cockDescript(y));
	...
	if (flags[kFLAGS.TIMES_CAUGHT_URTA_WITH_SCYLLA] <= 1) outputText("pay for the door and ");
...
internal function urtaAndRaphaelSurprise():void {
	clearOutput();
	outputText("Letting your lips split into a mischievous grin, you lean across the table and whisper into Urta's fluffy ear.  \"<i>Why don't you meet me in one of the back rooms with the lights off in about thirty minutes?  I'll see if I can get a friend, we can make a party of it.</i>\"");
