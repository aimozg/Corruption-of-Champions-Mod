...
internal function sophieSprite():void {
	if (flags[kFLAGS.SOPHIE_BIMBO] > 0 && flags[kFLAGS.SOPHIE_DISABLED_FOREVER] == 0) spriteSelect(SpriteDb.s_sophieBimbo);
	else spriteSelect(SpriteDb.s_sophie);
}

override public function bimboSophie():Boolean {
	return (flags[kFLAGS.SOPHIE_BIMBO] > 0 && flags[kFLAGS.SOPHIE_DISABLED_FOREVER] == 0 && flags[kFLAGS.SOPHIE_DEBIMBOED] == 0);
}
...
public function bimboSophieLustStickSurprise():void {
	...
	if (select == 0) {
		...
		if (!player.hasCock()) {
			...
			outputText("You shiver, " + player.sMultiCockDesc() + " growing hard and ready to submit to the feathery blond's love.  Perhaps you'll take care of that soon...");
			dynStats("lus", 15);
		}
		...
		sophieSprite();
		outputText("During an idle moment, you feel heavy breasts squeezing against your back.  Sophie caresses your " + player.chestDesc() + " and nibbles at your ear.  She asks, \"<i>Can we like, fuck soon, babe?  I've been so wet here all by myself, just stroking my pussy and dreaming of you.</i>\"  You can't help but turn and smile at her, but as soon as you do, she plants a kiss full on your lips.  ");
		if (!player.hasCock()) {
			outputText("Pushing her back, you give her ass a slap and mouth, 'Soon'.");
			dynStats("lus", 1);
		}
		else {
			outputText("Pushing her back, you try not to make contact against her with " + player.oMultiCockDesc() + ".  Your lips tingle dangerously, and you know you'll be lusting after harpies for at least a few hours.");
			dynStats("lus", 15);
		}
		sophieScene.luststickApplication(5);
	}
		...
		sophieSprite();
		if (player.hasCock()) {
			outputText("While standing there, drinking a sip of water, strong hands pull down your " + player.armorName + ".  Plump lips wrap around " + player.oMultiCockDesc() + " and smother it in plush, warm slipperiness.  You look down at your bimbo harpy and sigh while " + player.oMultiCockDesc() + " grows harder in her lips.  She pops off and asks, \"<i>Like, wanna fuck?</i>\"  You sigh and try to resist the images of yourself mounting the harpy that flood your mind.  It might be time to fuck her soon...");
			dynStats("lus", 15);
		}
		else {
			dynStats("lus", 10);
		...
		if (player.hasCock()) {
			dynStats("lus", 15);
			outputText(player.SMultiCockDesc() + " seems to wiggle inside your " + player.armorName + ".  Damn, she feels really good on you!  Maybe you should screw the slutty bimbo soon?  ");
		}
...
private function bimboSophieLicksRugs():void {
	...
	else outputText("You confidently smile and scoot your " + player.hipDescript() + " forward, anticipating the feeling of your bimbo-bitch's fat, pussy-licking lips on your slit.");
	...
	else if (player.getClitLength() < 7) {
	...
	if (player.getClitLength() < 3) outputText("hums on your clit and doubles the speed of her licking while you watch her finger-fucking her sloppy cunt.");
...
private function bimboSophieSixtyNineAfterCunnilingus():void {
	...
	if (!player.hasCock() || player.cockThatFits(sophieCapacity()) == -1) {
		...
		outputText("Sophie drops down onto her elbows, her ass-length blond plumage tickling at your " + player.legs() + " as she examines your groin.  The slutty harpy leans over your gold-smeared crotch and gives your " + player.clitDescript() + " a kiss, her plush lips smothering the ");
		if (player.getClitLength() < 1) outputText("tiny ");
		...
		if (player.getClitLength() >= 3) outputText(", even pausing to jack it off like a penis");
	...
	//SixtyNine Continuation - DUDES
		...
		outputText("As her imperatives roll through you, you shudder and shake.  " + player.OMultiCockDesc() + " immediately releases a dollop of pre-cum, and Sophie dives down on it like an addict seeking her fix.  The return of her drugged suction is nearly too much for you, and you feel your " + player.cockDescript(x) + " swelling in her mouth, the " + player.cockHead(x) + " thickening against her hungry tongue.  She shakes on top of you and releases another wave of syrupy fluids.  You swallow a mouthful, compulsively drinking her prodigious pussy's copious lady-spunk.  Your back arches, your " + player.legs() + " clench, and you erupt inside your bimbo's puckered, sucking mouth-hole, giving her tonsils a bath of your creamy semen.  Rivulets of semen run over her juicy lips as you cum too fast for her to keep up with.");
		if (player.cumQ() >= 500) {
			outputText("  She gulps and swallows as fast as possible, but cum starts oozing from her nose, dripping from the corners of her mouth, and spurting back out around your " + player.cockDescript(x) + " to spatter your groin.");
...
private function buttFuckBimboSophie():void {
	...
	outputText("You squeeze her shuddering rump hard enough to feel her flesh bulging out around your fingers, the soft cheeks nearly subsuming your hands, and then you pull her hard, dragging your " + player.cockDescript(x) + " the rest of the way inside her, all the way to the ");
...
private function fuckBimboSophiesTits():void {
	...
	outputText("She wobbles forward and her massive mounds easily slide to either side of " + player.oMultiCockDesc() + ", completely engulfing it.  Still holding on to her nipples, you pull them together and ");
	...
	outputText("One of the dumb blonde's hands circles under her bouncing, jiggling chest to stroke your " + player.cockDescript(x) + ", jacking you off with the spit, sweat, and drugged gloss as lubricants.  She jerks you hard and fast, smearing it ");
	if (player.balls > 0) outputText("all over your balls whenever she needs a break, and you feel your " + player.ballsDescriptLight() + " growing heavier by the moment.");
...
private function sophieImpregnationSex():void {
	...
	if (y >= 0) outputText("  A sinful tightness clenches your " + player.cockDescript(y) + " at the same time.  You accidentally started double penetrating her!  The sloppy vaginal heat around your second member is enough to make you forget that it was even an accident.");
...
private function letSophieMilkYoDick():void {
	...
	if (player.cocks[x].cockLength <= 30) {
		...
		outputText(" once you're dumped out every drop.  She touches your " + player.cockDescript(x) + " affectionately, tickling at its underside, and her flushed face seems extraordinarily pleased to have made you cum so hard.  Sophie's hands stroke her belly slowly as she looks up at you, whispering, \"<i>You're like, so good to Momma Sophie.  Just be sure and cum for my twat again soon, okay?  It misses you.</i>\"");
	}
	//On top of her.
	else {
		outputText("  Sophie");
...
public function sophiesEggHatches():void {
	...
	if (flags[kFLAGS.SOPHIE_ADULT_KID_COUNT] < 4) {	
		...
		if (flags[kFLAGS.SOPHIE_ADULT_KID_COUNT] == 0) {
			...
			if (flags[kFLAGS.SOPHIE_BIMBO] > 0) outputText(", though her lips are a little larger than you'd think was normal - maybe she inherited it from her mom?");
			else outputText(".");
		...
		//-Not that curvy, but has human feet and purple feathers. (violet eyes)
			...
			if (flags[kFLAGS.SOPHIE_BIMBO] > 0) flags[kFLAGS.DAUGHTER_THREE_BIMBO] = 1;
		}
		//Fourth Harpy:
		//-Curvy like Sophie, but with well-developed wings and extra-soft feathers (WINGJOBS).  Pink plumage.  (blue eyes)
			...
			if (flags[kFLAGS.SOPHIE_BIMBO] > 0) outputText(" like her mother used to have before her bimbofication");
			...
			if (flags[kFLAGS.SOPHIE_BIMBO] > 0) flags[kFLAGS.DAUGHTER_FOUR_BIMBO] = 1;
		}
	...
	else
	{
		...
		if (rand(2) == 0) outputText("pink");
			...
			if (flags[kFLAGS.SOPHIE_BIMBO] > 0) outputText("blonde");
...
public function sophieKidMaturation():void {
	...
	//Fifth+ daughter
		...
		if (flags[kFLAGS.SOPHIE_BIMBO] > 0) outputText("bimbo ");
