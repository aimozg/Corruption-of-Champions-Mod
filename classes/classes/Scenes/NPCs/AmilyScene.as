...
package classes.Scenes.NPCs
{
	...
	public class AmilyScene extends NPCAwareContent implements TimeAwareInterface
	{
		...
		*/
		...
		public function setCockLength(length:Number):void {
		...
		public function doRemoveCock():void {
			...
			flags[kFLAGS_AMILY_HAS_BALLS_AND_SIZE] = 0;
		}
		public function doGrowBalls():void {
			flags[kFLAGS_AMILY_HAS_BALLS_AND_SIZE] = 1;
		}
		public function modBallsSize(delta:Number):void {
			flags[kFLAGS_AMILY_HAS_BALLS_AND_SIZE] += delta;
		}
		...
		public function hasBalls():Boolean {
			return getBallsSize() > 0;
		}
		public function getBallsSize():Number {
			return flags[kFLAGS_AMILY_HAS_BALLS_AND_SIZE];
		}
		...
		public function encounterAmily():void {
			...
			if (flags[kFLAGS.AMILY_PC_GENDER] == player.gender) {
				...
				//GIRL MEETINZ
					...
					if (flags[kFLAGS.AMILY_CONFESSED_LESBIAN] == 2 && !hasCock()) {
				...
				default: //Amily is not pregnant
					...
					//[High Affection]
						...
						if (player.hasCock()) {
							...
							if (hasCock() && player.pregnancyIncubation == 0) outputText(" Or have you come to get knocked up?");
						}
						else if (player.hasVagina()) {
							if (hasCock() && player.pregnancyIncubation == 0) outputText("Have you come back so I could stuff another bun in your oven?");
		...
		private function amilyHighAffectionSecks():void {
			...
			//(If Amily is herm:
			if (hasCock()) outputText("  You can feel her erection, hot and solid, pressed between your two bodies.");
			outputText("\n\n");
			...
			if (!hasCock()) outputText("sucking on her sensitive clit");
			...
			//(if Amily is herm:
			if (hasCock()) outputText(" and cum spurts into the air between you, splattering on you both");
			outputText(".\n\n");
		...
		private function fuckAmilyPreg():void {
			...
			if (hasCock()) outputText("  The erection tenting her pants isn't helping.");
			outputText("\n\n");
			...
			if (hasCock()) outputText(", her own mouse-cock spewing futa-cum all over her belly");
			outputText(".\n\n");

			if (hasCock()) outputText("\"<i>Remember when you said this cock was a bad thing?</i>\" you tease, causing Amily to blush and playfully punch you in the shoulder.  \"<i>Sh-shut up!  It's... it's pretty incredible, there, I said it.</i>\"\n\n");
		...
		private function amilyCorruptSexMenu():void {
			...
			if (player.gender > 0) {
				...
				if (player.hasVagina()) {
					...
					if (hasCock()) addButton(3, "GetPen'ed", corruptAmilyCampBonesPCWithHerCock, null, null, null, "Have Amily put her cock to good use by penetrating you vaginally.", "Get Penetrated");
		...
		private function amilyAppearance():void {
			...
			if (isPureFollower()) {
				...
				if (hasCock()) {
					...
					outputText("She has a " + getCockLength() + "-inch penis growing from her crotch, replacing the clitoris that would normally jut from her vulva. It is surprisingly human looking, " + dickDesc + "");
					if (!hasBalls()) outputText(" - it even has a foreskin, not a sheath.");
					else outputText(", though it does have a" + ((isNotFurry()) ? " little sheath.":" fuzzy little sheath.") + "");
					outputText("\n\n")
				}
				if (hasBalls()) {
					outputText("She has " + amilyBalls() + " dangling ");
					if (hasCock()) outputText("underneath her shaft");
				...
				//(If Amily has a penis:
				if (hasCock()) {
					outputText("She has a half-erect " + getCockLength() + "-inch penis growing from her crotch. It is surprisingly human looking, "+((isFurry()) ? "naked and hairless ":"") +"and pink and throbbing");
					if (!hasBalls()) outputText(" - it even has a foreskin, not a sheath.\n\n");
					else outputText(", though it does have a" + ((isNotFurry()) ? " little sheath.":" fuzzy little sheath.") + "\n\n");
				}
				if (hasBalls()) {
		...
		private function fuckTheMouseBitch():void {
			...
			if (player.hasCock() || (player.hasVagina() && hasCock())) {
				//Make babies option
		...
		private function takeChargeAmilyFuck():void{
			...
			//(if Amily is herm:
			if (hasCock()) outputText("  Her pants begin to tent as her male organ reacts, and you start to idly stroke it as well for emphasis.");
			outputText("\n\n");
			...
			//(if Amily is herm:
			if (hasCock()) outputText(" and her " + amilyCock() + " standing erect");
			...
			if (hasCock()) outputText(" her " + amilyCock() + " spasming and spraying futa-jism in between your bodies");
			outputText(".\n\n");
		...
		private function takeChargeAmilyEatOut():void {
			...
			//(if herm:
			if (hasCock()) outputText(", and her " + amilyCock() + " is sticking up where her clitoris should be.  Y");
			...
			if (!hasCock()) outputText("her clitoris.");
			...
			//(if herm:
			if (hasCock()) outputText(" as her penis erupts and sprays its own cum into the air to sprinkle the both of you");
			outputText(".\n\n");
		...
		private function takeChargeAmilyGetSucked():void {
			...
			//(If Amily is herm:
			if (hasCock()) outputText("She turns halfway back to you as she goes. \"<i>I hope you'll remember this and return the favor someday,</i>\" she calls out to you. She then resumes walking off.");
			doNext(camp.returnToCampUseOneHour);
			player.orgasm('Generic');
			dynStats("sen", -1);
		}
		...
		private function takeChargeAmilyMountHer():void {
			...
			if (player.longestCockLength() >= 18) {
				outputText("Amily grabs hold of your " + player.cockDescript(player.longestCock()) + ", eagerly cramming it between her " + amilyTits() + ", mashing them together and running them up and down against your " + player.cockDescript(player.longestCock()) + ", surrounding it with warm, soft, velvety flesh.");
		...
		private function letAmilyLead():void {
			...
			if (hasCock()) choices[choices.length] = 4;
			//Get fucked by amily if she's herm and u have cunt
			if (hasCock() && player.hasVagina()) choices[choices.length] = 5;
			//========================================
			//CHOOSE PROPER SCENE
			...
			//B3: Eat Out
				...
				//(if Amily is herm:
				if (hasCock()) outputText("  Her cock bobs merrily in your field of vision.");
				outputText("\n\n");
				...
				//(If Amily is herm:
				if (hasCock()) outputText("\"<i>Could you... could you give it a lick, please?</i>\" she whispers, pointing at her male addition. Wanting to make her happy, you kiss the tip of her penis, eliciting a squeak of shock, before you start to lick from the base of her " + amilyCock() + " to the tip again. You then return your attention to her vagina itself.\n\n");
				...
				//(if Amily is herm:
				if (hasCock()) outputText("  Her penis erupts and sprays its own cum into the air to sprinkle the both of you.");
				outputText("\n\n");
				...
				//(if Amily is herm:
				if (hasCock()) outputText("and limp cock ");
			...
			//Let Amily Lead: Amily Mounts You
				...
				outputText("Amily situates herself between your legs and starts to stroke your " + player.vaginaDescript() + "; needless to say, her skill and experience from her own efforts to masturbate herself soon have you bucking with pleasure, lubing up your passage with your own feminine fluids.");
				//(if player is herm:
				if (player.hasCock()) outputText("  She strokes your " + player.cockDescript(0) + " a few times, but in a prefunctory manner; it's obvious that she's focusing on her own penis, this time.");
				outputText("\n\n");
		...
		private function giveAmilyAPresent():void {
			...
			if (hasCock() && (player.hasItem(consumables.PINKEGG) || player.hasItem(consumables.L_PNKEG))) {
		...
		public function giveAmilyPureIncubusDraft():void {
			...
			if (isPureFollower()) {
				...
				if (hasCock()) {
					...
					if (
						(getCockLength() >= maxSizePure && !flags[kFLAGS.HYPER_HAPPY]) ||
						(getCockLength() >= maxSizeHypr && flags[kFLAGS.HYPER_HAPPY])
						) {
		...
		private function amilyDickGrow():void {
			...
			//((getCockLength() >= 23 && flags[kFLAGS.AMILY_WANG_GIRTH] >= 5) && flags[kFLAGS.HYPER_HAPPY])
			var maxSize:Number;
			if (flags[kFLAGS.HYPER_HAPPY])
			{
				maxSize = 23;
			...
			else if (getCockLength() < 15 || flags[kFLAGS.HYPER_HAPPY]){
				modCockLength(1);
				modCockGirth(1 + ((getCockLength()-4)/9));
			}
			else if (getCockLength() >= maxSize) {
				setCockLength(maxSize);
				setCockGirth(3);
			}
		}
		...
		public function giveAmilyPurifiedSuccubusMilk():void {
			...
			//CORRUPT UNZ
				...
				//[(If Amily has a dick)
				if (hasCock()) outputText("  Her cock hardens with delight at your idea.");
				outputText("\n\n");
		...
		private function amilyDrinksSuccubusDelight():void {
			...
			if (!hasBalls()) {
				if (isCorrupt()) {
					...
					if (hasCock()) outputText("Her cock grows hard, but nothing else seems to happen...");
					outputText("\n\n");
					...
					if (hasCock()) outputText("right under her cock. Her skin grows and covers the base of her cock, pulling it in; giving her what looks like a sheath. It continues to expand and is finally complimented by a couple of orbs falling into her " + ((isFurry()) ? "fuzzy " :"") + "sack, giving it the weight it needs to produce more cum. ");
		...
		private function amilyBalls():String {
			...
			//size!
			if (getBallsSize() > 1 && rand(3) <= 1) {
				if (getBallsSize() < 2) desc += "large ";
				else if (getBallsSize() < 3) desc += "baseball-sized ";
				else if (getBallsSize() < 4) desc += "apple-sized ";
				else if (getBallsSize() < 5) desc += "grapefruit-sized ";
				else if (getBallsSize() < 7) desc += "cantaloupe-sized ";
				else if (getBallsSize() < 9) desc += "soccerball-sized ";
				else if (getBallsSize() < 12) desc += "basketball-sized ";
				else if (getBallsSize() < 15) desc += "watermelon-sized ";
				else if (getBallsSize() < 18) desc += "beachball-sized ";
		...
		public function amilyCock():String {
			...
			if (int(Math.random()*3) == 0) {
				if (getCockLength() < 3) descript = "squat ";
				if (getCockLength() >= 3 && getCockLength() < 5) descript = "short ";
				if (getCockLength() >= 5 && getCockLength() < 7) descript = "average ";
				if (getCockLength() >= 7 && getCockLength() < 9) descript = "long ";
				if (getCockLength() >= 10 && getCockLength() < 13) descript = "huge ";
				if (getCockLength() >= 13 && getCockLength() < 18) descript = "massive ";
				if (getCockLength() >= 18 && getCockLength() < 30) descript = "enormous ";
				if (getCockLength() >= 30) descript = "towering ";
				descripted = true;
			}
		...
		private function corruptAmilyOralSuckOff():void {
			...
			outputText(" " + player.cockDescript(0) + " throb one last time before saying, \"<i>Here's your reward, bitch.</i>\" Then you explode deep into her throat, much to her pleasure.");
		...
		private function corruptAmilyScissorsLikeAPro():void {
			...
			//[(if Amily has a cock)
			if (hasCock()) outputText(" Amily's " + amilyCock() + " throbs,");
			...
			if (hasCock()) {
				...
				//(if Amily has balls)
				if (hasBalls()) outputText(" and " + amilyBalls() + ", which you shift to reveal her ");
				else outputText(" before dipping down to her ");
			}
			else outputText("eyeing her ");
			outputText(player.vaginaDescript() + ".  You lift a hand and stroke the slutty box with your fingers.  It doesn't take long before she is wet and ready, her little clit poking out as stiff as a cock");
			//[(if Amily has a cock)
			if (hasCock()) outputText(" almost as if trying to match its bigger brother above");
			...
			//[(if Amily has a cock)
			if (hasCock()) outputText("and her cock ");
			...
			//[(if Amily has a cock)
			if (hasCock()) outputText(" Her " + amilyCock() + " throbs and cum splashes on her breasts and face.");
			outputText("\n\n");
		...
		private function corruptAmilysPussyGetsMotherfuckingFucked():void {
			...
			//[(if Amily has balls)
			if (hasBalls()) outputText("  Her balls do get in the way, but they form a nice cushion to rub your " + player.cockDescript(0) + " on, further stimulating you.");
			...
			//(if Amily is herm:
			if (hasCock()) outputText("  Her " + amilyCock() + " slices through the air like a fleshy knife, that stiff with blood and pleasure.");
			...
			//(If Amily is herm:
			if (hasCock()) outputText("  It doesn't take long - seconds, at most - before her pulsing, twitching cock finally explodes, spewing gush after gush of herm-spunk all over her tits and face, which Amily eagerly laps up.");
			outputText("\n\n");
		...
		private function corruptAmilyCampBonesPCWithHerCock():void {
			...
			//[(if Amily has balls)
			if (hasBalls()) outputText("  A pair of balls hangs under Amily's shaft; as you look, you have the slightest impression that her balls became rounder, fuller...");
			outputText("  Amily's shaft is ");
			if (getCockLength() < 6) outputText("small");
			else if (getCockLength() < 9) outputText("average");
			else outputText("big");
			...
			if (hasBalls()) outputText(", and her balls seeingly inflate");
			...
			//(if Amily has balls:
			if (hasBalls()) outputText(" and the meaty smack-smack-smack of her balls slapping against your " + player.assDescript());
			outputText(".\n\n");
			...
			//[(if Amily has balls)
			if (hasBalls()) outputText("The slapping of her balls on your ass reverberates around you.  ");
		...
		private function corruptAmilyBuckFutter():void {
			...
			if (hasCock()) outputText("; her " + amilyCock() + " stands erect, a tiny bead of pre forming on the tip as it throbs");
			outputText("\n\n");
			...
			outputText("You slap her ass roughly, causing it to jiggle, and yell, \"<i>Tighter!</i>\" Amily complies by constricting your " + player.cockDescript(0) + " with her ass muscles as tight as she can. In response, you up the rhythm of your thrusting. Dirty sounds of slapping complete the erotic picture, and you feel your ");
			//[(if PC has balls)
			if (player.balls > 0) outputText("balls churn and your ");
			...
			outputText("as she looks at your still erect " + player.cockDescript(0) + "; some cum still leaking from it. She licks her lips and inches her way towards you, still holding her ass closed; she begins licking the remaining cum off of your " + player.cockDescript(0));
			//[(if PC has balls)
		...
		public function amilyPopsOutKidsInCamp():void {
			...
			//CORRUPT!
				...
				//(If Amily is herm:
				if (hasCock()) outputText("  She has hold of her cock with both hands and is furiously jacking herself off, spurts of cum spilling like a fountain; then spattering onto her heaving breasts, face and belly, even though her erection refuses to die away.");
				//[If Amily has balls:
				if (hasBalls()) outputText("  Her testicles shake and quiver, swaying in the breeze as the pussy behind them readies itself to disgorge the life crammed into her swollen stomach above.");
				...
				//[(if Amily has a dick)
				if (hasCock()) outputText(" followed by a spurt of cum");
				...
				//[(if Amily has balls)
				if (hasBalls()) outputText(" it grabs Amily's balls and uses them to try and pull itself out;");
		...
		private function talkWithCORRUPTCUNT(sexAfter:Boolean = false):void {
			...
			//Conversation: How did she survive?
				...
				//[(if Amily has a dick)
				if (hasCock()) outputText(", and cock hardening");
		...
		private function rapeCorruptAmily3Female():void {
			...
			if (player.getClitLength() >= 4) outputText("Amily then crushes your sensitive clit between her breasts, working her mounds over it like it was a cock. You gasp and moan as Amily smiles up at you, happy to see your reaction to her stimulation.");
			//(else)
		...
		private function rapeCorruptAmily4Female():void {
			...
			if (player.getClitLength() >= 4) outputText("  One of her hands begins stroking your erect clit as if it was a cock. The pleasure of the act nearly overloads your senses.");
			outputText("\n\n");
		...
		public function amilyDefurryOfferAtCamp():void
		{
			...
			if (isCorrupt())
			{
				...
				case 2: // Female
					...
					break;
				default: // Fallback
					outputText("hardening " + player.nippleDescript(0) + "s ");
					break;
				}
		...
		private function liqueurUpTheWaifus():void {
			...
			if (player.hasCock()) outputText(", already feeling a strong stirring in your " + player.multiCockDescriptLight());
		...
		public function threesomeAmilUrtaCAWKS():void {
			...
			//if Futamily:
			if (hasCock()) outputText(" as her cock orgasms, too, spurting a nice, thick trail of mousecum all over Urta's face and tits");
			outputText(".\n\n");
		...
		public function pureAmilyPutsItInYourRectumDamnNearKilledEm():void {
			...
			if (player.analCapacity() < 30) outputText("Bracing yourself a bit, you grimace a little as you feel Amily start to push against your " + player.assholeDescript() + ", just the tip sliding in with a bit of resistance.");
			...
			else outputText("Amily lets out a slightly surprised squeak as your " + player.assholeDescript() + " offers her little resistance.");
			...
			if (player.hasCock()) {
				outputText("  You can feel her hand slipping down underneath you, gripping your " + player.cockDescript(x) + " gently and starting to stroke and tease it with her fingertips.");
				if (player.cockTotal() > 1) outputText("She brushes along " + player.sMultiCockDesc() + " one at a time, stroking for a moment or two before switching off.");
			}
			outputText("\n\n");
			...
			if (player.hasCock()) {
				...
				outputText("recum dribbles from your " + player.multiCockDescriptLight());
			}
			if (player.gender > 0) outputText(", forming a small puddle underneath you as you feel your climax approaching.");
			outputText("  With a tremendous shudder, you orgasm, your " + player.assholeDescript() + " clenching around her member.  At the same time, Amily moans into your ear, warm ribbons of herm-jizz filling your innards.");
			if (player.hasCock()) {
				outputText("  " + player.SMultiCockDesc() + " twitches with pleasure, soaking the ground beneath you with ");
		...
		private function fuckPureAmilysHeiny():void {
			...
			if (hasCock()) outputText("  Her pants tent from arousal as her male organ reacts to your sensual touching, and you start to teasingly stroke it as well.");
			outputText("\n\n");
			...
			if (player.cumQ() <= 250) outputText("your " + player.cockDescript(x) + " twitching and throbbing as you paint her insides with a few strands of cum");
			...
			if (hasCock()) outputText("her own member twitch and dribble a small puddle of clear pre-cum onto the ground below her, joining with ");
		...
		private function amilyEggStuff():void {
			...
			//if amily = herm
			if (hasCock()) outputText("and you notice a growing bulge in her trousers");
		...
		private function layEggsInAmilysCorruptedHole():void {
			...
			if (hasCock()) outputText(" and her erect shaft shuddering as it throbs in the air");
			...
			if (hasCock()) outputText("and stiff cock ");
			...
			if (player.hasCock()) {
				...
				if (hasCock()) outputText(", her shaft oozing thick globs of pre-cum that roll down her cock to smear against your own as she moves against you");
				outputText(".");
			}
			//(Females + AmilyUndicked)
			else if (player.hasVagina() && !hasCock()) {
			...
			//{(Genderless)
				...
				if (hasCock()) outputText(", her shaft covering your stomach with thick strands of pre-cum");
		...
		public function amilyLaysEggsLikeABitch():void {
			...
			if (!hasCock()) outputText("teases her clit");
			...
			if (!hasCock()) outputText("pussy visibly quivering");
		...
		public function amilySwimFuckIntro():void {
			...
			if (flags[kFLAGS.AMILY_TIMES_SWIMFUCKED] == 0) {
				...
				if (hasCock()) outputText("  D-don't laugh, okay?");
				outputText("</i>\"");
				...
				if (hasCock()) {
					...
					if (hasBalls()) outputText("  It snaps back onto her " + amilyBalls() + " to snug them tightly.");
				}
			...
			//Repeat Swim Intro
				...
				if (hasCock()) {
					...
					if (hasBalls()) outputText("  Her balls are snugly secured in her bottoms, looking like a cute little sphere of virile fun.");
				}
		...
		private function amilySwimFuckPartII():void {
			...
			if (!hasCock()) outputText("engorged bud that is her clit");
			...
			if (hasCock()) outputText(", and she climaxes all over again, spraying the underside of her " + amilyTits() + " and your [chest] with milky mouse seed");
		...
		private function amilyNurseCheckupV2(repeat:Boolean):void {
			...
			if (hasCock()) outputText("  It bulges out lewdly in a sheer outline of her " + amilyCock() + ", which is curled over to remain inside the stretchy skirt.");
			if (getCockLength() > 9) outputText("  The effort is a failure, as her immense cock is simply too big for such a slinky covering, and the fat tip curves down to dangle below the hem, pulsing dangerously.");
			...
			if (hasCock()) outputText(" that flings sticky ropes across your [chest].  Her back arches as she continues to launch her own jism towards you");
