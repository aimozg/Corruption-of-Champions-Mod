...
//const MARBLE_MILKED_BEFORE:int = 1165;
		...
		public function timeChange():Boolean
		{
			...
			if (player.hasStatusEffect(StatusEffects.MarblesMilk) && player.findPerk(PerkLib.MarblesMilk) < 0) {
...
private function rapeMarbleInHerRoom():void {
	...
	if (player.cocks.length > 0) {
		outputText("Chuckling to yourself, you free your " + player.multiCockDescriptLight() + " from your " + player.armorName + ".  You spend a moment to ask the helpless cow-girl if she is ready, her only response being a whimper, before ");
...
public function addictedEncounterHappy(clearS:Boolean = true):void {
	...
	//Return visits
		...
		//Addiction event version 2:
			...
			menu();
			addButton(0, "Marble", marbleChoreHelpChooseMarble);
			addButton(1, "Milk", marbleChoreHelpChooseMilk);
			addButton(2, "Refuse", marbleChoreRefusal);
		}
	}
}
...
private function standardSex(newpage:Boolean = true):void {
	...
	if (player.gender > 0) {
		...
		//(player is not wearing fetish gear)
			...
			outputText("You smile in return and begin removing your " + player.armorName + " from your body.  You start by slowly slipping off your top to reveal your " + player.allBreastsDescript() + ", ");
...
private function marbleCampSexNew():void {
	...
	clearOutput();
			...
			if (player.biggestTitSize() > 3) {
				outputText("pressing them against your own " + player.biggestBreastSizeDescript() + ".  She giggles slightly as she puts one of them between her mounds and rubs them up and down around it, teasing your " + player.nippleDescript(0) + " with her own " + marbleNips() + ", a trickle of milk leaking out onto your locked chests.");
			} else {
			...
			if (player.cocks.length > 0) {
				...
				if (player.cocks.length > 1) {
					outputText("  Yet still unsatisfied, Marble engulfs and rub the rest of your " + player.multiCockDescriptLight() + " within her ample mounds.  ");
				}
			}
			//If Player has a vagina, Marble titfucks that
			if (player.vaginas.length > 0) {
				outputText("\n\nShe slides down your body to rub and caress your " + player.vaginaDescript(0) + " with her breasts, sometimes pushing one of her " + marbleNips() + " inside your " + player.vaginaDescript(0) + " and pulling a small gasp from you.  ");
				//If clit is too small to be titfucked...
				if (player.getClitLength() < 2) {
					outputText("  Marble runs down each of your legs once with her breasts before unexpectedly plunging two of her fingers inside your " + player.vaginaDescript(0) + ".");
				} 
				else {
					outputText("  Marble then pulls your " + player.clitDescript() + " between her breasts and does her best to pump and stimulate it as though it were a cock.");
				}
			}
			//If Player is genderless, Marble improvises
			if (player.gender == 0) {
				outputText("  She then says, \"<i>I think I've found a way to make you cum, sweetie,</i>\" with a sly expression.  She moves down from your upper body and starts rubbing and caressing your lower body too. It is extremely arousing, but it doesn't seem to be enough to push you over the edge.  Marble stands up, kisses you and rubs a single finger against the smooth skin of your groin.  You are so close to going over the edge.  Then one of her fingers pushes up against your " + player.assholeDescript() + ", causing you to go rigid from the new stimulation.");
			}
			//Sweet release
			...
			if (player.vaginas.length > 0) {
				...
				if (player.vaginas[0].vaginalWetness < VAGINA_WETNESS_WET) outputText("  As you recover, you see that a small amount of your girly fluids has leaked onto Marble's breasts.");
				else if (player.vaginas[0].vaginalWetness <= VAGINA_WETNESS_SLICK) outputText("  As you recover, you see that Marble has been covered fairly liberally with your girly fluids.");
...
public function marbleBadEndFollowup():void {
	...
	if (morph=="human") {
		...
		outputText("\"<i>I'm so glad to have found a friend like you in this world, " + player.short + ". ");
	}
	//that's no human!
	else {
		...
		outputText("\"<i>You really are " + player.short + "!  I can't believe you left your humanity behind, but I'm glad to have found a friend in this world.  ");
	}
...
public function marbleNightSleepFlavor():Boolean {
	...
	if (player.gender == 0) {
		...
		if (player.lust < 33 && flags[kFLAGS.MARBLE_LUST] < 20) outputText("  After a few moments of getting comfortable, you drift off to sleep.");
			...
			//[Ask][Don't Ask]
			menu();
			addButton(0, "Ask", atNightAskMarbleForSomeSexMaybe);
			addButton(1, "Don't Ask", playerMenu);
			return true;
		}
		outputText("  ");
	}
	...
	//Replaces laying down texts when Marble is at or over 50 lust
		...
		if (player.lust < 33) {
			...
			if (player.isNaga() && flags[kFLAGS.MARBLE_DICK_LENGTH] > 0) {
				addButton(2, "Tailjob", giveMarbleTailjobRelease);
			} else {
				addDisabledButton(2, "Tailjob", "This scene requires you to have naga tail. Marble should have a cock.");
			}
		}
		else {
...
private function marbleCuddlin():void {
	...
	if (!pcPregWithMarblesKids() && !marblePregWithPCKids()) {
		...
		if (player.tallness <= 78) outputText("pulls your head on top of her chest.  She idly strokes your " + player.hairDescript() + " and says, \"<i>Sweetie, how about I be your pillow for the night?</i>\"  You happily agree, and soon you drift off to the sound of her steady breathing while you lay between her sizable breasts.");
...
private function marbleNightSexIntro(clear:Boolean = true):void {
	...
	if (player.gender == 3) {
		if (player.cockArea(x) <= marbleCuntCapacity()) {
			...
			menu();
			addButton(0, "Dick", marbleNightSexDudes);
			addButton(1, "Vagina", marbleNightSexChicks);
		}
		else {
...
private function marbleNightSexDudes():void {
	...
	flags[kFLAGS.MARBLE_LUST] = 0;
	clearOutput();
	outputText("You gently lift yourself up and slide the tip of your " + player.cockDescript(x) + " into her waiting hole.  Ever so slowly, you push yourself further and further in.  Each inch gained brings gentle moans and coos from Marble, encouraging you onward.  ");
	...
	if (player.cor + player.lib < player.statusEffectv4(StatusEffects.Marble) + 10) {
		...
		//Fun, the PC's breasts fall onto Marble's Face!  If they have them, anyway...
			...
			if (player.biggestTitSize() >= 4) {
				outputText("landing your " + player.biggestBreastSizeDescript() + " into Marble's face!  You hear a slight muffled cry underneath you as Marble takes her hands off your hips and lifts you up off of her.  ");
			} 
			else {
	...
	if (player.cumQ() < 250) {
		outputText("Marble cries out in joy as your " + player.cockDescript(x) + " unleashes its load within her insides.  ");
	} 
	else {
...
private function marbleNightSexChicks():void {
	...
	clearOutput();
	if (flags[kFLAGS.MARBLE_DICK_TYPE] == 0) {
		outputText("You gently lift yourself up as Marble produces her double dildo and slips it inside her womanhood.  Smiling at her, you slowly lower your body and impale your " + player.vaginaDescript(0) + " onto the artificial rod.  ");
		...
		outputText("The two of you cry out in orgasm as your " + player.vaginaDescript(0) + " clamps down on one end of her dildo and her womanhood clamps down on the other end.  ");
		//Does this PC have a cock too?
		//If (PC has at least 1 cock)
		if (player.totalCocks() > 0) {
			outputText("At the same time, your " + player.multiCockDescriptLight() + " sprays its own load onto Marble's chest and stomach.  ");
		}
	...
	//PC is female, Marble is a herm
		...
		player.slimeFeed();
		outputText("You gently lift yourself up into the air and guide your " + player.vaginaDescript(0) + " to the tip of her waiting " + marbleCock() + ".  You slowly lower your " + player.vaginaDescript(0) + " onto her quivering " + marbleCock() + ", gaining inch by inch ");
		//is she satisfied?
		...
		outputText("The two of you cry out in orgasm as your " + player.vaginaDescript(0) + " clamps down on her " + marbleCock() + " as it fills you with its seed.  ");
		//Does this PC have a cock too?
		if (player.totalCocks() > 0) {
			outputText("At the same time, ");
			if (player.totalCocks() > 1) outputText("each of ");
			outputText("your " + player.multiCockDescriptLight() + " sprays its own load onto Marble's chest and stomach.  ");
		}
		outputText("The two of you give one more shudder from the wonderful stimulation, before collapsing on top of one another.  ");
