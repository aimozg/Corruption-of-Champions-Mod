...
internal function marbleFightWin():void {
	...
	if(player.hasPerk(PerkLib.Feeder) || player.lactationQ() > 200) feed = forceFeedMarble;
...
private function applyMarblesMilk():void {
	...
	var sec:StatusEffectClass = player.statusEffectByType(StatusEffects.MarblesMilk);
	if(!sec) {
...
private function canGiveItem():Boolean {
	...
	else if (player.hasPerk(PerkLib.MarblesMilk)) return true;
	return false;
}
...
public function giveItem():void {
	...
	else if (player.hasPerk(PerkLib.MarblesMilk)) addButton(0, "Lactaid", giveMarbleLactaid);
