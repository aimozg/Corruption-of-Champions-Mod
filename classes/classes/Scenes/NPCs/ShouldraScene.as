...
//const GHOST_GIRL_SLIME_X_SHOULDRA_COUNTER:int = 510;
		...
		public function timeChange():Boolean
		{
			...
			if (shouldraFollower.followerShouldra()) {
                ...
                if (flags[kFLAGS.SHOULDRA_PLOT_COUNTDOWN] > 0 && CoC.instance.model.time.hours == 3) flags[kFLAGS.SHOULDRA_PLOT_COUNTDOWN]--;
            }
			return needNext;
		}
...
internal function shouldraGreeting():void {
	...
	if(flags[kFLAGS.TIMES_MET_SHOULDRA] < 3) {
		...
		//([if first time] 
		if(flags[kFLAGS.TIMES_MET_SHOULDRA] == 0) outputText("\"<i>You're the asshole that took my racks, aren't you?  I was hoard-...er, I was keeping an eye on those!</i>\" she accuses, eyes narrowing angrily.  ");
		outputText("Finally, she acknowledges your readiness to fight. With a sigh and a shrug, she raises her arms and drops into an orthodox boxing stance. Once again, you heft your [weapon].");
	...
	//(after three encounters with her)
		...
		if(flags[kFLAGS.TIMES_BEATEN_SHOULDRA] >= 3) {
			...
			//([if first time] 
			if(flags[kFLAGS.TIMES_BEATEN_SHOULDRA] == 3) outputText("You can't say you saw that coming.  It seems she has offered to forego the fight in lieu of sexings.  ");
...
private function novemberAlphaHotel():void {
	spriteSelect(66);
	clearOutput();
	outputText("You shake your head slowly, raising your [weapon] as you regard her.  \"<i>Ah, so that's how you want it?</i>\" she titters, a smile gracing her features.  \"<i>In that case, enough talk... have at you!</i>\"\n\n");
	startCombat(new Shouldra());
}
...
internal function defeatDannyPhantom():void {
	...
	//(defeat by lust)
		...
		outputText("The girl glances down at her leggings, then does a double take, realizing as if for the first time she was completely overwhelmed by her lust. She sinks to her knees, dragging her breeches down and pressing a hand against her pubic mound.  ");
		if(flags[kFLAGS.TIMES_POSSESSED_BY_SHOULDRA] < 1) outputText("Strangely, the action seems to have little to no effect.");
		else outputText("Of course, the motion has no effect on her libido, due to her ghostly state.");
		outputText("  Looking up at you with what can only be described as desperation, she pants, \"<i>Please, hero... I need release... let me in?</i>\"  You take a step forward, stroking your chin softly in contemplation.  ");
		//([if first time]
		if(flags[kFLAGS.TIMES_POSSESSED_BY_SHOULDRA] < 1) {
			outputText("What does she mean, \"<i>let me in?</i>\" Do you want to find out?");
...
private function noSlimingSlimer():void {
	spriteSelect(67);
	clearOutput();
	outputText("You shake your head angrily at the brazen girl, ");
	//([if first time] 
	if(flags[kFLAGS.TIMES_POSSESSED_BY_SHOULDRA] == 0) outputText("not feeling up to heed the strange girl's whims.");
	else outputText("mystified at the thought of letting a ghost possess your body.");
	outputText("You whirl about and stomp away.  She rises, screaming, \"<i>Fine! I'll go and take over a fucking goblin, again!</i>\" and storms the other direction. Before you move out of earshot, you hear a sniffle from the departing ");
	//([if first time]
	if(flags[kFLAGS.TIMES_POSSESSED_BY_SHOULDRA] == 0) outputText("girl (what was that about her 'taking over' a goblin?)");
	else outputText("spirit");
	outputText(", and a twinge of guilt runs through you.");
	cleanupAfterCombat();
}
...
private function littlePigLittlePigLetMeIn():void {
	spriteSelect(67);
	clearOutput();
	outputText("You decide ");
	if(flags[kFLAGS.TIMES_POSSESSED_BY_SHOULDRA] == 0) outputText("you can afford being a little adventurous with the obviously desperate girl");
	else outputText("having a spiritual tag-along might be a bit of fun");
	outputText(", and agree to her request to possess you. She staggers to her feet and undresses, panting and shivering with unfulfillable sexual need. You reach out to her, drawing her");
	//[if <~7' tall]
	if(player.tallness < 84) outputText(" into your arms for a tight hug");
	else outputText(" up into your arms for a tight hug");
	outputText(". The young woman heaves a happy sigh, then begins mumbling an incantation into your shoulder. Soon enough, the girl's body turns translucent and she begins to sink into you, planting a little peck on your cheek on her way in.  ");
	//([if first time] 
	if(flags[kFLAGS.TIMES_POSSESSED_BY_SHOULDRA] == 0) outputText("Wait... WHAT!?  Did you just let some sort of... g-... g-... g-... G-... G-... GHOOOOOST possess you?");
...
private function nowOnVickiLake():void {
	...
	if(ooze != null && shark != null) {
		outputText("You recall the experiences of both the slime and the shark girl. Which encounter would you wish to seek out?");
...
private function sharkbustersVictory():void {
	...
	outputText("You don't even give her time to finish, rushing up to her and slamming your lips against hers in a passionate kiss");
	//[if breasts larger than flat]
	if(player.biggestTitSize() >= 1) outputText(", [allbreasts] squishing and squashing against her own barely-clad bosom");
	...
	if(player.hasCock() && player.hasVagina()) {
		outputText("Which gender would you like to focus on?");
		//male / female)
...
private function sharkyEctoginas():void {
	...
	//[if repeat]
	if(flags[kFLAGS.TIMED_SHARKGINAS] > 0) outputText(" again...");
	outputText("</i>\" she pants, brow suddenly furrowed with worry.  ");
	if(flags[kFLAGS.TIMED_SHARKGINAS] == 0) outputText("You begin to ask what she's talking about, but the words die in your mouth when");
	else outputText("You can only shake your head resignedly as");
	...
	if(player.hasKeyItem("Deluxe Dildo") >= 0) outputText(" by the biggest object you've experienced since Tamani's deluxe dildo");
	...
	//[if breasts present]
	if(player.biggestTitSize() >= 1) outputText("and caking your cleavage with the excess lubrication");
	...
	outputText("Some time later, with another, much softer kiss, you draw the spirit back into yourself, leaving a very tired and surprisingly content shark girl in her wake. She reveals her consciousness throughout the experience, going so far as to apologize ");
	//(if repeated: again) 
	if(flags[kFLAGS.TIMED_SHARKGINAS] > 0) outputText("again ");
...
private function ghostIzmaPenis():void {
	spriteSelect(67);
	clearOutput();
	outputText("You swiftly disrobe and lay your [armor] to the side, revealing your stiffening [cocks] in all of its glory. The ghost-girl's eyes brighten at the sight, and she even allows herself a little swoon (which looks absolutely silly, given her current tough shark-like vessel) as she regards your manhood.  ");
	outputText("She staggers forward, dropping to her knees in front of you");
	//[if testicles present] 
	if(player.balls > 0) outputText(" and cradling your [balls] with almost reverent gentleness, running her coarse shark-skinned fingers across your sack as gently as she can manage");
	outputText(".\n\n");
	...
	//[if first time]
	if(flags[kFLAGS.TIMES_SHARKPENISED] == 0) outputText("whose expression reveals she's as clueless as you are.");
	else outputText("who only smiles as she realizes her 'mistake' in her spellcasting once again.");
	outputText("  \"<i>That wasn't nearly what the spell should have been,</i>\" the girl muses, licking her lips ");
	if(flags[kFLAGS.TIMES_SHARKPENISED] == 0) outputText("absently as she contemplates what could have caused the lengthening of your loins");
	else outputText("in anticipation for the experience you both are about to undergo, yet again");
	outputText(". Your jaw drops as her lips puff out, suddenly looking much more sultry and far fuller than before. Your spiritual seductress swiftly takes note, as well, and her lips curl up into a mischievous smile ");
	if(flags[kFLAGS.TIMES_SHARKPENISED] == 0) outputText("as she deduces the nature of the spell. \"<i>I think... oh my... my host's saliva may have expansive qualities on... sexual aspects of the body,</i>\" she explains. \"<i>So, if I were to do this...</i>\"");
	else outputText("\"<i>Would you look at that, my magic spittle still works! How about I just...</i>\"");
	outputText("\n\n");
	
	outputText("Before you can stop her, she positions her head over your twitching shaft and opens her mouth wide. Out pours a torrent of drool amassed from a mixture of her arousal and undoubtedly magical influence to splatter against your maleness");
	if(player.balls > 0) outputText(" and softly churning sack");
	...
	if(player.balls > 0) outputText("  Your enchanted testes give an audible groan as you reach climax.");
	else outputText("  You feel something stirring in your groin, and you squirm as the tingling intensifies.");
...
private function ghostGooGurlzDuckfaces():void {
	...
	//([if penis present] 
	if(player.hasCock() && !player.hasVagina()) outputText(" presenting your [cocks] to the spectral slime.");
	else if(player.hasVagina()) outputText(" slipping a hand down to your " + clitDescript() + " eagerly.");
	//[if herm] 
	else outputText(" both of your sexes throbbing in excitement.");
...
private function slimeGinaFlation():void {
	...
	//([if corruption <50]
	if(player.cor < 50) outputText("you beg her to stop before this gets out of hand.");
	...
	else outputText("Your frame is jostled about as the slime under you shrinks, your goo-filled gut pushing your [allbreasts] up into your overburdened jaw.");
	outputText("\n\n");
	...
	outputText("Content in taking second place, your anus also stretches as the bulb flows into it. Moments later, before you are even allowed to recover from the first two, your jaw is forced farther open to make room for the bulbous deposit. You hum unhappily as your throat widens, not unlike a croaking frog. Your already-huge belly burgeons with the new additions until it's at least twice as large as ");
	if(player.hasStatusEffect(StatusEffects.MeetWanderer)) outputText("both of Markus's testicles, combined!");
	else outputText("a wheelbarrow!");
	outputText("\n\n");
	...
	else outputText("arms, [legs], and tail");
	...
	if(player.biggestTitSize() < 1) outputText("see your " + nippleDescript(0) + " wiggling sporadically.");
	else outputText("see your [allbreasts] trembling, " + nippleDescript(0) + " twitching eagerly.");
...
private function slimeyPenorsAhoy():void {
	...
	if(player.balls > 0) outputText("and teasing your quaking testes ");
	...
	outputText("Even if you were to attempt to assist her with the lovemaking, at this point, the ghost girl is too out of it to realize. Keeping your arms and legs pinned, she vigorously goes at her work, her extra-large ass ");
	if(player.isNaga()) outputText("slapping against your tail");
	else if(player.isGoo()) outputText("smooshing against your goo");
	else outputText("swiftly bruising your thighs");
	outputText(".  A bestial groan rumbles from the goo-ghost as she continues to ride you, and despite everything, you begin to enjoy the treatment, going so far as to begin bucking your hips in rhythm.");
	if(player.balls > 0) outputText("  The near-vacant-minded girl rewards you with two slime-tentacles to fondle your scrotum.  You hiss in a sharp breath at the sudden stimulation, but swiftly come to welcome it.");
	...
	if(player.balls > 0) outputText("the heat spreads to your testicles, promptly puffing them up with arcane virility");
	else outputText("you feel a discomfort beginning in your prostate, as if the organ is beginning to grow with arcane virility");
	outputText(". Once more, she hops upon you, beginning a ferocious assault against your genitals. Looking up at her face, however, she does not seem very satisfied with the predicted output. Although not surprised, you're nonetheless horrified when she begins casting the same spell, and then a third time.  ");
	if(player.balls > 0) outputText("Your [legs] are forced apart by your burgeoning cum-sack");
	else outputText("You nearly go insane with the pressure upon your sex");
	outputText(", but she doesn't even notice, too excited by the notion of being so filled up.\n\n");
	
	outputText("In your current overfilled state, it takes a mere two minutes of her frenzied ministrations to cause your ");
	if(player.balls > 0) outputText("balls to churn");
	else outputText("body to prepare itself for release");
	...
	if(player.balls > 0) outputText("waddle over to where the mewling ghost girl is twitching on the ground; your now-shrinking testicles hindering your gait.");
	else outputText("limp over to where the mewling ghost girl is twitching on the ground; your now-shrinking prostate hinders your gait.");
...
private function genderlessGhostBuster():void {
	spriteSelect(67);
	clearOutput();
	outputText("Excited to help relieve the girl of her accumulating lust, you allow her control of your arms. She eagerly cups a hand against your crotch to feel... nothing. Confused, she quickly wriggles out of your [armor] and goes in for another grope, feeling nothing but skin. \"<i>Son of a...please don't tell me you don't have any sexual bits I can play with!</i>\" she huffs, and you chuckle sheepishly, not knowing what to tell her. She groans and falls back onto your " + buttDescript() + ", muttering a string of curse words under your breath. \"<i>That's it!</i>\" she fumes. \"<i>I'm going to get off, whether you want me to or not!</i>\" She resumes cursing and wiggling your hips for a bit longer, then stops suddenly, a smirk playing across your face. \"<i>Whether you want me to or not...</i>\"\n\n");
	
	outputText("She hops to your feet and makes for the decrepit hut you first saw her in.  ");
	if(flags[kFLAGS.SHOULDRA_GENDERLESS_FUCK_COUNT] == 0) outputText("You raise an eyebrow, genuinely curious as to how she'll figure out how to relieve her desire.  ");
	...
	if(flags[kFLAGS.SHOULDRA_GENDERLESS_FUCK_COUNT] == 0) outputText("Curious as to its contents, you glance inside—it's a chest of dildos, of every shape and size. Before you can ask why the hell a ghost would carry a CHEST of dildos, ");
	else outputText("You recall distinctly the contents of that chest, and are not surprised ");
	...
	if(player.biggestTitSize() < 1) outputText("two new additions bud on your chest.");
	else if(player.biggestTitSize() <= 8) outputText("your [allbreasts] begin to bounce all around.");
	else outputText("your [allbreasts] swell slightly to make room for a sudden increase in milk production.");
	outputText("As the ghost continues darting around the room, ");
	//[if flat to E-cup]
	if(player.biggestTitSize() <= 8) outputText("your new EE-cup breasts settle");
	else outputText("the lactation continues, and the ghost girl moves back to the table and settles down a bit");
	...
	if(!player.hasFuckableNipples()) outputText("new ");
	else outputText("even larger ");
	...
	if(player.ass.analLooseness < 4) outputText(" and stretching your butt obscenely");
	outputText(".");
	...
	outputText("You chuckle to yourself, admittedly impressed by her prowess. A pressure lifts from your chest, and you absently raise a hand to find ");
	if(player.biggestTitSize() < 1) outputText("your tits, along with the elongated nipples, receding back into a flat chest.");
	else if(player.biggestTitSize() <= 8) outputText("your boobflesh shrinking back down into its normal size, nipples included.");
	else outputText("the nipples shortening once again, and the lactation finally stabilizing.");
	outputText("  With a happy sigh, you heave yourself upright, slipping back into your [armor] and moving out into the ruins once more.");
...
private function penisLossThatIsntALoss():void {
	...
	if(flags[kFLAGS.SHOULDRA_PENIS_DEFEAT_TIMES] == 0) outputText("She abruptly gasps in excitement and begins moving your hands towards your groin, though you swiftly reclaim control. \"<i>You have a penis,</i>\" she telepathically pulses to you with an anxious and excited edge to her voice. \"<i>My hosts... they rarely... they...</i>\"");
	else outputText("She chuckles softly as she recalls the times you've shared, and she hungrily goes for your maleness once more, though you throw her off of it.");
	...
	//[if intelligence <50] 
	if(player.inte < 50) outputText("It's not very hard for her to break down your fortifications, and you soon feel yourself being pushed aside by her vast mental capacity.");
		...
		if(player.cockTotal() > 1) {
			...
			else outputText("two other shafts and pumps them vigorously.");
		}
		outputText("\n\n");
		...
		outputText("Although her mouth is quite full, her voice rings clearly in your head as she starts casting a spell.  You glance down at her, surmising from her almost serene expression and heavy-rimmed eyes that she's even gotten over her inability to orgasm.  She finishes her spell with the usual flourish before once more focusing her attention to your pleasure.  ");
		if(player.cockTotal() == 2) outputText("Taking her hand away from your other dick, the ");
		...
		outputText("Taking advantage of your distraction, your former mind slave pushes you over and jumps on top of you.  Her impotently sopping box leaves streaks of clear fluid across your belly as she continues her work on your cock");
...
private function ghostBJFinisher(inside:Boolean):void {
	...
	if(inside) {
		...
		outputText("You watch her move off into the ruins with a new spring in her step, chuckling as she goes before heading back to camp yourself.");
	}
...
private function penisGartuanGhostSmexVictory():void {
	...
	outputText("\"<i>Hold up,</i>\" the demon requests, suddenly far less boisterous. \"<i>You're sayin' you're a chick, right?  And not some chick with a dick");
	if(player.hasVagina()) outputText(" like this bitch here");
	outputText("?</i>\"\n\n");
	...
	if(flags[kFLAGS.SHOULDRA_EXGARTUAN_SPIRIT_SEX_COUNT] > 0) outputText("once again ");
...
private function hermaphroditeGhostsCumEctoplasm():void {
	...
	var x:Number = player.biggestCockIndex();
	clearOutput();
	if(flags[kFLAGS.SHOULDRA_HERMSEX_COUNT] == 0) outputText("She abruptly gasps in excitement and begins moving her hands towards your groin, though you swiftly reclaim control of the unruly appendages. \"<i>You have a penis,</i>\" she telepathically pulses to you with an anxious and excited edge to her voice. \"<i>My hosts... they rarely... they... </i>\"  ");
	//[if repeated] 
	if(flags[kFLAGS.SHOULDRA_HERMSEX_COUNT] > 0) outputText("She chuckles softly as she recalls the times you've shared, and she hungrily goes for your maleness once more, though you mentally throw her off of it.  ");
	outputText("The communication suddenly breaks as the ghost-girl's will surges against yours, all thoughts of submissiveness vanishing from her mind.  ");
	if(player.inte < 50) outputText("It's not very hard for her to break down your fortifications, and you soon feel yourself being pushed aside by her vast mental capacity.");
	...
	outputText("Taking complete control of your body as if it was never actually yours, she removes your [armor] and throws them carelessly to the ground. Grabbing up your " + cockDescript(x) + " in both hands and fondling it awkwardly, she strikes you as obviously inexperienced with the male anatomy, despite her levels of intelligence. She continues to grope your lengthening member, admitting, \"<i>Even with the rare male bodies I have possessed, I rarely got to play with their...</i>\"\n\n");
	...
	if(player.balls > 0) outputText("Stopping her double-handed stroke technique, she slips your hand under your obtrusive [balls] and gently pushes them aside to get a better look at your previously unnoticed female sex.  ");
...
internal function loseToShouldra():void {
	...
	//[if dick] 
	if(player.hasCock()) outputText("Each pulse pushes her a bit further onto your hardening manhood");
	else if(player.hasVagina()) outputText("You can't deny the heat slowly welling up in your loins, your love-tunnel getting more lubricated by the moment");
	...
	outputText("Wondering what the point of that was, you raise your head and glance up at her face, now at least four inches higher that she just was. She reaches down and gives your ");
	if(player.hasCock()) outputText("quivering member");
	else if(player.hasVagina()) outputText("sopping gash");
...
private function ourDadTaughtUsNotToBeAshamedOfOurDicks():void {
	...
	var x:Number = player.biggestCockIndex();
	//[first encounter] 
	if(flags[kFLAGS.SHOULDRA_PENIS_DEFEAT_TIMES] == 0) outputText("She abruptly gasps in excitement, ");
	else outputText("She 'ooo's softly as she recalls your anatomy, ");
	outputText("and you feel your hands moving unbidden towards your crotch. You snap the sneaky extremities back to your sides, knowing exactly what she wants to do. The ghost girl soon proves herself the stronger in the contest of wills, however, and eventually, your will breaks and she takes complete control once again. She forces your body to stand, removing your [armor] and throwing them carelessly to the ground. She takes your " + cockDescript(x) + " in both hands and fondles it awkwardly, obviously inexperienced with the male anatomy.  \"<i>Most of my hosts have been women,</i>\" she explains as she continues to grope your lengthening member.  \"<i>And even then, I rarely got to play with their...</i>\"\n\n");
...
private function loseToShouldraWithWorms():void {
	spriteSelect(67);
	//[first encounter]
	if(flags[kFLAGS.SHOULDRA_WORM_SCENE_COUNTER] == 0) outputText("She abruptly gasps in excitement, ");
	else outputText("She 'ooo's softly as she recalls your anatomy, ");
	outputText("and you feel your hands moving unbidden towards your crotch. The ghost girl soon proves herself the stronger in the contest of wills, however, and eventually, your will breaks and she takes control. She heaves your body off the ground, removing your [armor] and throwing them carelessly to the ground. She takes your [cock] in both hands and fondles it awkwardly, obviously inexperienced with the male anatomy. \"<i>Most of my hosts have been- WHAT THE FUCK!?</i>\"\n\n");
...
private function ghostGinaLosses():void {
	...
	if(flags[kFLAGS.SHOULDRA_VAGINAL_POSSESSIONS] == 0) outputText("She 'hmm's several times, raising your hand and tapping your finger against your chin. She snaps your fingers as a revelation washes over her.  \"<i>Of course!</i>\" she cries excitedly.  \"<i>I actually have never done this before!</i>\"");
	else outputText("\"<i>Oh well, I suppose I can always just do that again,</i>\" she muses. \"<i>It worked well enough the first time.</i>\"");
	outputText("\n\n");
	
	outputText("You are about to question exactly what she is referring to, but quickly fall silent as she slides out of your [armor] and idly strokes your cunt. She takes control of your mouth and chants a fairly lengthy spell, her absent-minded masturbation steadily gaining fervor as she progresses. With a final, shouted word of power, both of you cry out as your clitoris suddenly pulses and swells into your hand, the increased sensitivity nearly driving you insane. Your bitch-button does not stop there, however, puffing up with each heartbeat. Your eyes roll back in your head and small primitive grunts and moans escape your lips as your clit engorges in the ghostly magic, not stopping until it reaches at least four times its original length. A breeze blows through the ruined rooftops, and you scream as the wind brushes against your elongated bitch-button. Too sensitive... the thing is way too sensitive!\n\n");
...
private function loseToShouldraAsHerm():void {
	spriteSelect(67);
	if(flags[kFLAGS.SHOULDRA_HERMSEX_COUNT] == 0) outputText("She abruptly gasps in excitement, ");
	else outputText("She coos softly as she recalls your anatomy, ");
	outputText("and you feel your hands moving unbidden towards your crotch. You snap the sneaky extremities back to your sides, knowing exactly what she wants to do. The ghost girl soon proves herself the stronger in the contest of wills, however, and eventually, your will breaks and she takes complete control once again. She forces your body to stand, removing your [armor] and throwing them carelessly to the ground. She takes your [cock] in both hands and fondles it awkwardly, obviously inexperienced with the male anatomy.  \"<i>Most of my hosts have been women,</i>\" she explains as she continues to grope your lengthening member.  \"<i>And even then, I rarely got to play with their...</i>\"\n\n");
	...
	if(player.balls > 0) outputText("Stopping her double-handed stroke technique, she slips your hand under your obtrusive [balls] and gently pushes them aside to get a better look at your previously unnoticed female sex.  ");
	outputText("She chuckles evilly, and you groan, mind cycling through the possible outcomes of the girl's impulses. Your lips curl unbidden in an evil smile, then begin mouthing an archaic incantation, and you wince inwardly, bracing for the worst. She finishes and goes back to the vigorous stroking, and you are left to wonder what the spell was meant to do—that is, until you feel a tingling beginning at the base of your [cock], slowly climbing up the shaft until your entire length is covered in the sensation.\n\n");
...
private function shouldraGiantCockLoss():void {
	spriteSelect(67);
	var x:Number = player.biggestCockIndex();
	outputText("Her attention inevitably moves to your gigantic dick, somehow still contained in your garments, and she swiftly clamors to free it from its prison. Practically tearing off your [armor], she tilts your head back in order to fully take in your " + cockDescript(x) + ", a small line of drool escaping the edge of your mouth. She opens your mouth as if to speak, but cannot find words, instead settling for a long and anxious groan. Her first reaction is to simply hug the titantic shaft, squeezing happily. Eventually, she seems to realize her original intent and draws back, stroking your chin with your hand while she apparently ascertains the best way to stimulate your monster. With a small 'aha!', she struggles to stand, unused to such a strange center of balance. Bringing you along for the ride, she begins wandering the streets of the city, taking control of your mouth and shouting \"<i>My oh my, how on earth will I ever blow my load with this GIANT dick?</i>\" Her efforts are rewarded when you hear a small pitter-patter of feet, distantly at first, but hastily heading toward you. A duo of goblins rush out around a corner, take a moment to confirm the reported size of your manhood, and hurry over, undressing, tripping and shoving each other in an effort to win the race to win your attention. As they reach you the excited little creatures nearly bowl you over as they start stroking and licking every inch of cockflesh they can reach. \"<i>Girls!</i>\" the ghost exclaims sternly (with your voice), crossing your arms in front of you as they turn their heads to regard you. She allows a hint of a smile to show at the corners of your mouth as she continues, \"<i>Trust me when I say there will be enough of me for both of you.</i>\"\n\n");
	
	outputText("Without another word, she reaches out and grabs one of the goblins by the shoulder and begins chanting in her strange unknown language, wiggling the fingers of your absent hand at the ");
	//[if first time]
	if(flags[kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) outputText("bemused ");
	else outputText("excited ");
	outputText("girl. With a flair, she finishes the incantation, then moves to the other goblin and repeats the process. The first goblin, meanwhile, ");
	if(flags[kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) outputText("searches her body for any signs of change, and when she finds none, begins pouting.");
	else outputText("resists the urge to smash her tits together, waiting with as much patience as she can muster.");
	outputText("  By this time, the ghost girl has finished with the spell, ");
	//[if first time] 
	if(flags[kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) outputText("and instead of offering an explanation, she merely ");
	else outputText("and, to the green girl's obvious delight, ");
	outputText("reaches over and kneads the goblin's ");
	//([if first time]
	if(flags[kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) outputText("small, firm B-cups");
	else if(flags[kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 1) outputText("perky, sizeable C-cups");
	else if(flags[kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 2) outputText("well-formed, jiggly D-cups");
	else if(flags[kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 3) outputText("big and squishy E-cups");
	else if(flags[kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 4) outputText("oversized, heaving F-cups");
	else outputText("gigantic and openly-bared G-cups");
	outputText(". The goblin ");
	//[if first time] 
	if(flags[kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) outputText("instantly stops her protests, suddenly too stimulated by the massage to worry about any magical favoritism, ");
	else outputText("cries out in ecstasy at the still-incredible sensation, ");
	//[if first time]
	if(flags[kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) outputText("although that doesn't stop the second goblin to begin voicing her own dissent");
	else outputText("and you can't help but notice the excited blush running across the second goblin's face");
	outputText(". The ghost girl pinches a quickly-stiffening nipple and pulls it out, ");
	if(flags[kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) outputText("and... pulls the breast along with it?");
	else outputText("and pulls the breast along with it.");
	outputText("  You inwardly gasp ");
	//[if first time] 
	if(flags[kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) outputText("as the effects of the magic are revealed");
	else outputText("despite seeing the magic before, always enthralled by the process");
	outputText("; it has turned the goblin's breasts into an almost putty-like substance");
	if(flags[kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) outputText(", evidently!");
	else outputText(" once again");
	outputText(". The ghost girl begins to speed the process up, softly laying the goblin on the ground and kneading out the green girl's tits, adding at least half a cup size with every tug. The second goblin, watching intently behind your back, ");
	if(flags[kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) outputText("gasps in amazement, ");
	else outputText("can't restrain her urges any longer, ");
	outputText("swiftly reaching down to roughly grab at her own ");
	//[if first time] 
	if(flags[kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) outputText("petite boobs");
	else if(flags[kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 1) outputText("modest bosom");
	else if(flags[kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 2) outputText("sizable chest");
	else if(flags[kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 3) outputText("large endowments");
	else if(flags[kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 4) outputText("huge tits");
	else outputText("titanic melons");
	outputText(", crying in excitement as her breasts also begin to billow out under her own ministrations.\n\n");
	
	outputText("The first goblin's breasts, at this time, have reached at least ");
	if(flags[kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) outputText("E-cups");
	else if(flags[kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 1) outputText("F-cups");
	else if(flags[kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 2) outputText("G-cups");
	else if(flags[kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 3) outputText("H-cups");
	else if(flags[kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 4) outputText("I-cups");
	else outputText("J-cups");
	...
	//[if first time] 
	if(flags[kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) outputText("HH-cups");
	//[if repeated once] 
	else if(flags[kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 1) outputText("II-cups");
	else if(flags[kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 2) outputText("JJ-cups");
	else if(flags[kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 3) outputText("KK-cups");
	else if(flags[kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 4) outputText("LL-cups");
	else outputText("MM-cups");
	outputText(", and the ghost girl turns your attention to the second goblin, whose boobs are embarrassingly uneven due to her...unpracticed skills. The ghost girl moves you over to her, mercifully evening the currently ");
	//([if first time] 
	if(flags[kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0) outputText("DD-cup (and C-cup, on the other side)");
	else if(flags[kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 1) outputText("EE-cup (and D-cup, on the other side)");
	else if(flags[kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 2) outputText("FF-cup (and E-cup, on the other side)");
	else if(flags[kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 3) outputText("GG-cup (and F-cup, on the other side)");
	else if(flags[kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 4) outputText("HH-cup (and G-cup, on the other side)");
	else outputText("II-cup (and H-cup, on the other side)");
	...
	if(flags[kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] <= 4) outputText("they have kept at least a cup size of flesh from their experience");
	else outputText("their breasts seem to have reached their crescendo, keeping the same (admittedly incredibly stacked) measurements");
	outputText(", which they happily fondle as they wander away, side-by-side. Moving to your [armor] and redressing, you shake your head, wondering, not for the first time, if such a large appendage is truly beneficial to you, and wondering if this was the last you've seen of your two new goblin friends.\n\n");
	outputText("A strange sensation begins in your midsection, and you press a hand against the flesh curiously. You remove it swiftly as a familiar head suddenly pushes out, wiggling about until it faces you. She smiles, then draws her arms out of her torso and uses them as leverage (with a whispered spell to make her hands tangible) to remove herself from you completely. Muttering an incantation and solidifying her again-unremarkable form, she winks at you and gathers her tunic, slipping it over her frame. As you watch, she slips her thumb and forefinger into her satisfied snatch, producing a healthy helping of a spooky gooey substance. Reaching into a pocket inside her tunic, she draws out a small bottle, which she promptly squishes the ectoplasm into. She offers you the bottle, explaining, \"<i>The product of a... ghost climax, if you will. Keep it, it's good for you.</i>\"  With a happy goodbye, she wanders into an alley, grabbing her ruined leggings and discarded shoes as she goes, and disappears into the shadows. You move to leave, but find your [legs] growing heavy, fatigue washing over you in the aftermath of the possession. Before you know it, you're on your knees, and you have to struggle to keep your eyelids from drooping. You succumb to the exhaustion, slumping to the ground and soon snoring contentedly.");
...
private function genderlessShouldrasLossRapes():void {
	...
	outputText("You marvel at how resourceful the ghost-girl is, then pause and consider the ramifications of the eggs. Your suspicions are confirmed as ");
	if(player.biggestTitSize() < 1) outputText("two new additions bud on your chest.");
	else if(player.biggestTitSize() <= 8) outputText("your [allbreasts] begin to bounce all around.");
	else outputText("your [allbreasts] swells slightly to make room for a sudden increase in milk production.");
	outputText("  As the ghost continues darting around the room, ");
	if(player.biggestTitSize() <= 8) outputText("your new EE-cup breasts settle");
	else outputText("the lactation continues");
	...
	outputText("She smacks it ");
	if(player.isNaga()) outputText("against your naga-crotch");
	else if(player.isGoo()) outputText("into your cushiony goo");
...
private function ghostGartuanLossSceneOrSomeShit():void {
	spriteSelect(67);
	var x:Number = player.biggestCockIndex();
	outputText("Your entire body shivers in delight as the ghost girl regards your excessively large " + cockDescript(x) + ". She places your hand over the bulge in your [armor] and titters in glee as your member twitches and hardens, just a tad. She strips eagerly, but is halted by a familiar booming voice coming from the half-erect member. \"<i>WHO THE HELL IS THIS BITCH?</i>\" a perplexed Exgartuan yells grumpily. \"<i>Living in peoples' bodies is MY THING!</i>\"\n\n");
	...
	outputText("\"<i>Hold up,</i>\" the demon requests, suddenly far less boisterous. \"<i>You're sayin' you're a chick, right?  And not some chick with a dick");
	if(player.gender == 3) outputText("like this bitch here");
	outputText("?</i>\"\n\n");
...
private function superCoolPaladinSexStuffDudeGasmJizzMonkies():void {
	...
	outputText("You look up to gaze into her moderately attractive face. Is it?");
