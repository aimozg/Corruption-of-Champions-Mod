...
//- Player does not have a cock or balls, for now
		...
		public function timeChangeLarge():Boolean {
			if (checkedRussetRogue++ == 0 && model.time.hours == 6 && flags[kFLAGS.RAPHEAL_COUNTDOWN_TIMER] >= 0 && player.hasKeyItem("Camp - Chest") >= 0 && player.gems >= 5 && player.statusEffectv1(StatusEffects.TelAdre) >= 1 && flags[kFLAGS.IN_INGNAM] == 0) {
...
private function RaphaelDress():void {
	...
	outputText("Soon enough you put it down and begin to inspect the equally lustrous garment he left you. You unfold it and marvel at the Bordeaux colored clothes.");
	//nxt page
...
private function RaphaelSelectDate():void {
	...
	outputText("It almost feels like you're consenting when you reach for his soft, cushioned padded furry paw, after which he pulls you up and helps you over the wall.");
	//{Unlocks Picnic}
...
private function friskRejectChoice():void {
	...
	outputText("You laugh as he blushes, beating a sound retreat. You're sure this won't be the last you'll see of him however.");
	//{Game removal}
	flags[kFLAGS.RAPHEAL_COUNTDOWN_TIMER] = 14;
	flags[kFLAGS.REJECTED_RAPHAEL] = 1;
	doNext(playerMenu);
...
private function RaphaelPicnicEnd():void {
	...
	if (flags[kFLAGS.FUCK_OFF_THEIVING_RAPHAEL] == 0)
	{
		...
		outputText("You catch the object: he has returned the priceless ruby pendant. You recollect yourself and head back to camp.");
	}
	else
	{
		...
		outputText("You recollect yourself and head back to camp.");
	}
	doNext(playerMenu);
}
...
private function RaphaelThieverySmex():void {
	...
	outputText("Finally the fragile tension between you breaks.  Raphael undoes your zipper and roughly pulls your bodysuit down, revealing your [allbreasts]. You're stunned; the fine, abrasive texture of his nimble fingers' pads run in eager circles across your sensitive neck, from whence moans have begun to flow. His soft fur brushes across your body as the fox moves downwards.  Gingerly, he puts his maw down on your right breast and suckles once, but in the span of an agonizingly long time.  The " + nippleDescript(0) + " is sucked into his toothy mouth, which he puts down on the nub carefully.  The little points scrape by sensually, before your nipple slips through them.\n\n");
...
private function quiksilverFawkesEndGame():void {
	...
	outputText("It removes itself from atop your body.  After having the breath knocked out of you, you take a moment to recover it and finally manage to get back up and meet the two monsters of the mist.");
...
private function QuiksilverFawkesPtII():void {
	...
	outputText("Your heart jumps as Hati manages to discover a pile of Raphael's clothes, near the barrel.  It's irrefutable proof.  Raphael must have put them there to throw the wolves off.  Hati gives you a look of dissapointment.  <i>\"He's nude. They must have slept together.\"</i>");
	//({If player is wearing red bodysuit}
	if(player.armorName == "red, high-society bodysuit") outputText("  Suddenly, his eyes and ears perk.  <i>\"Isn't that the stolen clothing of the widow Camembert that she's wearing? You know, before she mysteriously dropped the charges after a nightly visit from the fox? Should we confiscate it anyway?\"</i>");
	outputText("\n\n");
	...
	outputText("You could sell Raphael out, or you could cover for him.  What do you do?");
...
private function coverForRaphael():void {
	...
	//({If cor is below 30}
	if(player.cor < 30) outputText("  You slowly shake your head, tortured by the trusting twinkle in the wolf's soft yellow eyes.");
	outputText("\n\n");
...
private function coverForRaphaelII():void {
	...
	outputText("You blink as he fades off into the mist and you're left to absorb the morning's events by yourself.  For a minute you're concerned with the wolves, but at least you've managed to keep Raphael as a friend.  Though, you are curious what he means by 'orphanage'.");
	//Concludify things
...
private function betrayRaphael():void {
	...
	outputText("You blink as he fades and you're left to reflect on these morning's events by yourself.  For a minute you're concerned for the fox, but the wolves don't seem like the types to harm him... much.  You are curious what they plan to do with him and what his 'sentence' might be, though.");
