		...
		override public function defeated(hpVictory:Boolean):void
		{
			...
			if(player.hasStatusEffect(StatusEffects.Feeder) && flags[kFLAGS.SFW_MODE] <= 0) {
				...
				if(player.lust >= 33 && player.gender > 0) {
					...
					EngineCore.simpleChoices("B.Feed",SceneLib.lake.greenSlimeScene.rapeOozeWithMilk,"Rape",SceneLib.lake.greenSlimeScene.slimeVictoryRape,"",null,"",null,"Leave",combat.cleanupAfterCombatImpl);
				}
				//Rapes not on the table.
				else {
					outputText("\n\nYour nipples ache with the desire to forcibly breastfeed the gelatinous beast.  Do you?");
					EngineCore.doYesNo(SceneLib.lake.greenSlimeScene.rapeOozeWithMilk,combat.cleanupAfterCombatImpl);
				}
			}
			//Not a breastfeeder
				...
				EngineCore.doYesNo(SceneLib.lake.greenSlimeScene.slimeVictoryRape, combat.cleanupAfterCombatImpl);
			}
			else combat.cleanupAfterCombatImpl();
		}
