		...
		public function giantGrabFail(struggle:Boolean = true):void {
			...
			if (struggle) {
				...
				if (player.str >= 80) {
					...
					if (!player.hasPerk(PerkLib.Juggernaut) && armorPerk != "Heavy") {damage = 10 + rand(str * 0.5);
					player.takePhysDamage(damage, true);
					}
				}
					...
					if (!player.hasPerk(PerkLib.Juggernaut) && armorPerk != "Heavy") {damage = 20 + rand(str * 0.75);
					player.takePhysDamage(damage, true);
					}
				}
