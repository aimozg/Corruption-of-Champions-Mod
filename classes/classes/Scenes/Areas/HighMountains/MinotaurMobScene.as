		...
		public function timeChange():Boolean
		{
			...
			if (flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00328] > 0) {
				...
				if (flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00328] <= 0 && flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00327] > 0) {
					...
					flags[kFLAGS.ADULT_MINOTAUR_OFFSPRINGS]++;
				}
			}
			//NEXT KID!
...
public function meetMinotaurSons():void {
	...
	if(flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00325] == 1) {
		...
		if(!player.minotaurAddicted()) {
			...
			if(flags[kFLAGS.ADULT_MINOTAUR_OFFSPRINGS] > 3) outputText("some of ");
	...
	//Repeat meetings
		...
		//Non tribe size
		if(flags[kFLAGS.ADULT_MINOTAUR_OFFSPRINGS] < 20) {
			//[Non Addicted Repeat] 
			if(!player.minotaurAddicted()) {
				outputText("It doesn't take long for your exploration into the mountains to bring you face-to-face with your minotaur offspring once again.  There're " + num2Text(flags[kFLAGS.ADULT_MINOTAUR_OFFSPRINGS]) + " of them here, arranged around you in a loose semicircle.  You can smell their familiar and arousing odor in the air; while it brings a tingle to your nethers, you're determined to resist it.  ");
			...
			//[Addicted Repeat] 
				...
				outputText("It doesn't take too long for your exploration into the mountains to bring you face-to-face with your minotaur offspring once again.  There're " + num2Text(flags[kFLAGS.ADULT_MINOTAUR_OFFSPRINGS]) + " of them here, arranged around you in a loose semicircle.  You can smell their familiar, arousing odor drifting around you, and though you want to be strong, there's nothing you want more than to bury your face beneath those loincloths and start slurping your reward for having such wonderful children.  ");
				if(player.wetness() <= 2) outputText("A trickle of wetness runs down your [leg] almost immediately.");
				else if(player.wetness() <= 4) outputText("A steady flow of your juices soaks your [legs] almost immediately, just from the scent invading your nostrils.");
				else outputText("A flood of wetness soaks your [armor] and [legs] with feminine drool almost immediately as your body reflects the obscene lust coursing through your veins.");
...
private function nonAddictMinotaurGangBang():void {
	...
	if(flags[kFLAGS.ADULT_MINOTAUR_OFFSPRINGS] >= 10) outputText("The assembled beasts snicker at the noise, stroking themselves slowly while awaiting a chance at your hungry holes.");
...
private function loseToMinoMobVeryPregnant():void {
	...
	//(short) 
	if(flags[kFLAGS.ADULT_MINOTAUR_OFFSPRINGS] < 8) {
...
private function analSpearSemiPregMinotaurGangbang():void {
	...
	if(player.hasFuckableNipples()) {
		...
		if(flags[kFLAGS.ADULT_MINOTAUR_OFFSPRINGS] < 7) outputText("shakes his head.  \"<i>You're only saving up trouble for later, little cunt.  Your hole after Mom's, remember.</i>\"  The minitaur wistfully withdraws his cock and crawls a few paces off, where he sits in a sulk.");
...
private function victoryMinotaurGangTitFuck():void {
	...
	//(FORK: Small or large)
	//(LARGE) 
	if(flags[kFLAGS.ADULT_MINOTAUR_OFFSPRINGS] >= 10) {
	...
	//(SMALL) 
		...
		if(flags[kFLAGS.ADULT_MINOTAUR_OFFSPRINGS] > 3) outputText("His glaze spurts up along the opening created by his larger brother's shaft, and soon he too is cumming, obediently orgasming for you.  ");
		...
		if(flags[kFLAGS.ADULT_MINOTAUR_OFFSPRINGS] < 5) outputText("The minotaurs let you get dressed while they sigh happily, clearly pleased with the outcome.");
		else {
			...
			if(flags[kFLAGS.ADULT_MINOTAUR_OFFSPRINGS] > 5) {
				outputText(" and his brother");
				if(flags[kFLAGS.ADULT_MINOTAUR_OFFSPRINGS] >= 6) outputText("s");
			}
...
private function victoryAllThePenetrationsMinotaurGangBang():void {
	...
	//(Nipplefuck PG:)
	if(player.hasFuckableNipples() && flags[kFLAGS.ADULT_MINOTAUR_OFFSPRINGS] > 4) {
	...
	if(player.hasFuckableNipples() && flags[kFLAGS.ADULT_MINOTAUR_OFFSPRINGS] > 4) outputText("your nipples squirt, ");
	...
	if(player.hasFuckableNipples() && flags[kFLAGS.ADULT_MINOTAUR_OFFSPRINGS] > 4) {
		if(flags[kFLAGS.ADULT_MINOTAUR_OFFSPRINGS] > 4) outputText("Meanwhile, your " + nippleDescript(0) + "s get a similar feeling from the studs arranged around them");
	...
	if(flags[kFLAGS.ADULT_MINOTAUR_OFFSPRINGS] < 20) outputText("gang");
...
private function victoryBJNippleFuckMinotaurGang():void {
	...
	if(flags[kFLAGS.ADULT_MINOTAUR_OFFSPRINGS] >= 7) outputText(", chased by jeers from the remainder of your brood");
	...
	if(flags[kFLAGS.ADULT_MINOTAUR_OFFSPRINGS] < 5) outputText("relax and fan themselves while giving you lusty looks");
