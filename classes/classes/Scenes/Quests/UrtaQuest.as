...
private function visitPCPreUrtaQuest(truth:Boolean):void {
	...
	if(flags[kFLAGS.ANEMONE_WATCH] > 0 && flags[kFLAGS.ANEMONE_KID] > 0)
		outputText("A confused-looking anemone with a " + CoC.instance.gameLibrary.findItemType(flags[kFLAGS.ANEMONE_WEAPON_ID]).longName + " nearly sees you, but you duck around a rock and escape her notice.  ");
...
//			doNext(combat.combatMenu);
	...
	if (player.hasPerk(PerkLib.HoldWithBothHands) && player.weaponName != "fists") damage *= 1.2;
	//Weapon addition!
...
//		doNext(combat.combatMenu);
	...
	if(!monster.hasStatusEffect(StatusEffects.Stunned) && !monster.hasPerk(PerkLib.Resolute) && damage > 0) {
