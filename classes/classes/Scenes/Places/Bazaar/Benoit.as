...
private function eggySuggest():void {
	...
	player.orgasm();
	if ((player.pregnancyType == PregnancyStore.PREGNANCY_OVIELIXIR_EGGS || player.hasPerk(PerkLib.HarpyWomb) || player.hasPerk(PerkLib.Oviposition)) && (player.pregnancyIncubation == 0 || player.pregnancyType == PregnancyStore.PREGNANCY_OVIELIXIR_EGGS)) {
...
private function tryToConvertToBassyWomb():void {
	...
	//Ingredients in inventory: 
		...
		//[No oviposition:
		if(!player.hasPerk(PerkLib.Oviposition)) outputText("  You feel slightly bloated, but otherwise fine; you sense that you can lay eggs of your own volition now.");
