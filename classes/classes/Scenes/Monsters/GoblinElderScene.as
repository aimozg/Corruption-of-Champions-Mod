...
public class GoblinElderScene extends BaseContent
	{
		...
		public function goblinElderRapeIntro():void
		{
			...
			if (flags[kFLAGS.SPARRABLE_NPCS_TRAINING] == 2) {
				...
				if (flags[kFLAGS.PRISCILLA_DEFEATS_COUNTER] == 1 && flags[kFLAGS.PRISCILLA_LVL_UP] < 1) {
					flags[kFLAGS.PRISCILLA_DEFEATS_COUNTER] = 0;
					flags[kFLAGS.PRISCILLA_LVL_UP] = 1;
				}
				if (flags[kFLAGS.PRISCILLA_DEFEATS_COUNTER] == 2 && flags[kFLAGS.PRISCILLA_LVL_UP] == 1) {
					flags[kFLAGS.PRISCILLA_DEFEATS_COUNTER] = 0;
					flags[kFLAGS.PRISCILLA_LVL_UP] = 2;
				}
				if (flags[kFLAGS.PRISCILLA_DEFEATS_COUNTER] == 3 && flags[kFLAGS.PRISCILLA_LVL_UP] == 2) {
					flags[kFLAGS.PRISCILLA_DEFEATS_COUNTER] = 0;
					flags[kFLAGS.PRISCILLA_LVL_UP] = 3;
				}
				if (flags[kFLAGS.PRISCILLA_DEFEATS_COUNTER] == 4 && flags[kFLAGS.PRISCILLA_LVL_UP] == 3) {
					flags[kFLAGS.PRISCILLA_DEFEATS_COUNTER] = 0;
					flags[kFLAGS.PRISCILLA_LVL_UP] = 4;
				}
				if (flags[kFLAGS.PRISCILLA_DEFEATS_COUNTER] == 5 && flags[kFLAGS.PRISCILLA_LVL_UP] == 4) {
					flags[kFLAGS.PRISCILLA_DEFEATS_COUNTER] = 0;
					flags[kFLAGS.PRISCILLA_LVL_UP] = 5;
				}
				if (flags[kFLAGS.PRISCILLA_DEFEATS_COUNTER] == 6 && flags[kFLAGS.PRISCILLA_LVL_UP] == 5) {
					flags[kFLAGS.PRISCILLA_DEFEATS_COUNTER] = 0;
					flags[kFLAGS.PRISCILLA_LVL_UP] = 6;
				}
				if (flags[kFLAGS.PRISCILLA_DEFEATS_COUNTER] == 7 && flags[kFLAGS.PRISCILLA_LVL_UP] == 6) {
					flags[kFLAGS.PRISCILLA_DEFEATS_COUNTER] = 0;
					flags[kFLAGS.PRISCILLA_LVL_UP] = 7;
				}
				if (flags[kFLAGS.PRISCILLA_DEFEATS_COUNTER] == 8 && flags[kFLAGS.PRISCILLA_LVL_UP] == 7) {
					flags[kFLAGS.PRISCILLA_DEFEATS_COUNTER] = 0;
					flags[kFLAGS.PRISCILLA_LVL_UP] = 8;
				}
				if (flags[kFLAGS.PRISCILLA_DEFEATS_COUNTER] == 9 && flags[kFLAGS.PRISCILLA_LVL_UP] == 8) {
					flags[kFLAGS.PRISCILLA_DEFEATS_COUNTER] = 0;
					flags[kFLAGS.PRISCILLA_LVL_UP] = 9;
				}
				if (flags[kFLAGS.PRISCILLA_DEFEATS_COUNTER] == 10 && flags[kFLAGS.PRISCILLA_LVL_UP] == 9) {
					flags[kFLAGS.PRISCILLA_DEFEATS_COUNTER] = 0;
...
//[DUDEGASM]
		private function gobboGetsRapedMaleFits():void
		{
			...
			if (player.cockArea(x) > monster.vaginalCapacity() * .8) {
				outputText("You pick up the defeated goblin, looking her over. She crosses her arms across her chest pitifully and asks, \"<i>What now?</i>\" with her eyes darting down when she thinks you won't notice. A grimace temporarily crossing her face at the size of your " + cockDescript(x) + ". You get the idea of giving her more cock than she can handle, remove her spider-silk loincloth, and lower her down towards your " + cockDescript(x) + ". The tip slips between her moist and folds, stretching her and taking some of her weight off your arms. She winces slightly, wrapping her legs as far around your " + hipDescript() + " as possible.\n\n");
			...
			//(FITS – Get ridden)
				...
				outputText("You pick up the defeated goblin, looking her over.  She crosses her arms across her chest pitifully and asks, \"<i>What now?</i>\" with her eyes darting down when she thinks you won't notice.  You muse to yourself 'great minds think alike', remove her spider-silk loincloth, and lower her down towards your " + cockDescript(x) + ".  The tip slips between her moist and parted folds, brushing against her entrance and taking some of her weight for you.  She goes cross-eyed and smiles happily, wrapping her legs as far around your " + hipDescript() + " as possible.\n\n");
...
//REQUIRES: AT LEAST ONE DICK AND A COPY OF ATLAS SHRUGGED - MUST NOT BE MONSTROUSLY HUGE
		private function gatsGoblinBoners():void
		{
			...
			if (CoC.instance.inCombat) {
				...
				outputText("You pick yourself back up, jerking yourself slowly as cum dribbles from your " + cockDescript(x) + " onto the collapsed body of the goblin.  The goblin gets up, wobbling from the ordeal. \"<i>Thanks for the seed, stud! I'll have new goblins to teach,</i>\" she says.");
