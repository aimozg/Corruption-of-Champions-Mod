...
public class EventParser {
    ...
    private static function goNextWrapped(time:Number, needNext:Boolean):Boolean {
        ...
        while (CoC.instance.timeQ > 0) {
            ...
            SceneLib.combat.manaRegeneration(false);
...
//	}
        ...
        if (((player.weaponPerk == "Large" && !player.hasPerk(PerkLib.TitanGrip)) || player.weaponPerk == "Dual" || player.weaponPerk == "Dual Large") && player.shield != ShieldLib.NOTHING) {
    ...
    //Used by UrtaPregs to stop updates in order to display a scene
    public static function escape():void {
        _escape = true;
    }
    private static var _escape:Boolean = false;
    private static function get parser_escape():Boolean {
        var r:Boolean = _escape;
        _escape = false;
        return r;
    }
    private static function pregnancyProgress():int {
        ...
        var conditions:/*PerkType*/Array = [
            PerkLib.MaraesGiftFertility,
            PerkLib.MagicalFertility,
            PerkLib.FerasBoonBreedingBitch,
            PerkLib.FerasBoonWideOpen,
            PerkLib.FerasBoonMilkingTwat
        ];
        var updates:int = 1;
        for each (var pType:PerkType in conditions){
            if(player.hasPerk(pType)){updates += 1;}
        }
        if (player.hasPerk(PerkLib.Diapause)) {
            if (flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00228] <= 0 || (player.pregnancyIncubation <= 0 && player.buttPregnancyIncubation <= 0)) {
                return 0;
            }
        ...
        for(var i:int = 0; i < updates; i++) {
            needNext ||= player.pregnancyAdvance();
            if(parser_escape){return 2;}
        }
        return needNext ? 1 : 0;
    }
        ...
        if (CoC.instance.model.time.hours == 2) {
            if (CoC.instance.model.time.days % 30 == 0 && flags[kFLAGS.ANEMONE_KID] > 0 && player.hasCock() && flags[kFLAGS.ANEMONE_WATCH] > 0 && flags[kFLAGS.TAMANI_NUMBER_OF_DAUGHTERS] >= 40) {
                ...
                if (player.gender > 0 && (!player.hasStatusEffect(StatusEffects.JojoNightWatch) || !player.hasStatusEffect(StatusEffects.PureCampJojo)) && (flags[kFLAGS.HEL_GUARDING] == 0 || !SceneLib.helFollower.followerHel()) && flags[kFLAGS.ANEMONE_WATCH] == 0 && (flags[kFLAGS.HOLLI_DEFENSE_ON] == 0 || flags[kFLAGS.FUCK_FLOWER_KILLED] > 0) && (flags[kFLAGS.KIHA_CAMP_WATCH] == 0 || !SceneLib.kihaFollower.followerKiha()) && !(flags[kFLAGS.CAMP_BUILT_CABIN] > 0 && flags[kFLAGS.CAMP_CABIN_FURNITURE_BED] > 0 && (flags[kFLAGS.SLEEP_WITH] == "Marble" || flags[kFLAGS.SLEEP_WITH] == "")) &&
                        flags[kFLAGS.IN_INGNAM] == 0 || flags[kFLAGS.CAMP_UPGRADES_MAGIC_WARD] == 2) {
    ...
    public static function growHair(amount:Number = .1):Boolean {
        ...
        var thresholds:Array = [1, 3, 6, 10, 16, 26, 40, Math.max(40, player.tallness)];
        ...
        for each (var t:Number in thresholds){
            if(player.hairLength >= t && currentLength < t){
    ...
    public static function growBeard(amount:Number = .1):Boolean {
        ...
        var thresholds:Array = [0.2, 0.5, 1.5, 3, 6];
        ...
        for each (var t:Number in thresholds) {
            if(player.beardLength >= t && tempBeard < t){
